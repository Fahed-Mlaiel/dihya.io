"""
Schemas Pydantic/GraphQL pour le module Santé (Django routes)
Ultra avancé, multilingue, sécurisé, compatible REST & GraphQL, RGPD-ready.
"""
from typing import Optional, List
from pydantic import BaseModel, Field, constr

class SanteProjectSchema(BaseModel):
    id: Optional[int] = Field(None, description="ID unique du projet Santé")
    name: constr(min_length=1, max_length=255) = Field(..., description="Nom du projet Santé")
    description: str = Field(..., description="Description du projet Santé")
...
class SanteProjectListSchema(BaseModel):
    projects: List[SanteProjectSchema]

import strawberry
@strawberry.type
class SanteProjectType:
    id: int
    name: str
    description: str
    created_by: int
