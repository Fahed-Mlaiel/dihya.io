"""
Schemas Pydantic/GraphQL pour le module Energie (Django routes)
Ultra avancé, multilingue, sécurisé, compatible REST & GraphQL, RGPD-ready.
"""
from typing import Optional, List
from pydantic import BaseModel, Field, constr

class EnergieProjectSchema(BaseModel):
    id: Optional[int] = Field(None, description="ID unique du projet Energie")
    name: constr(min_length=1, max_length=255) = Field(..., description="Nom du projet Energie")
    description: str = Field(..., description="Description du projet Energie")
...
class EnergieProjectListSchema(BaseModel):
    projects: List[EnergieProjectSchema]

# Pour GraphQL (exemple avec Strawberry)
import strawberry
@strawberry.type
class EnergieProjectType:
    id: int
    name: str
    description: str
    created_by: int
