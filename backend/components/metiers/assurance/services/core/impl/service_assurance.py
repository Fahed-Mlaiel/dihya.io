"""
service_assurance.py - Service principal assurance (Python)
...
def get_assurance_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleassurance_{model_id}", "data": "..."}


def list_assurance_models():
    return [get_assurance_model(i) for i in range(1, 4)]
