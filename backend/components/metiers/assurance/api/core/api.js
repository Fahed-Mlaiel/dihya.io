// api.js – Point d’entrée Express ultra avancé pour l’API Assurance
const express = require('express');
const router = express.Router();
const AssuranceController = require('../core/controllers/assurance_controller');
...
// GET assurance entity by ID
router.get('/assurance/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await AssuranceController.getById(req.params.id);
...
// POST create assurance entity
router.post('/assurance', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await AssuranceController.create(req.body);
...
// PUT update assurance entity
router.put('/assurance/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await AssuranceController.update(req.params.id, req.body);
...
// DELETE assurance entity
router.delete('/assurance/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await AssuranceController.delete(req.params.id);
