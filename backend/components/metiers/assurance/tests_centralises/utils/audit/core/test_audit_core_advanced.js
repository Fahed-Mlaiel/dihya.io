const { auditAssurance } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditAssurance({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditAssurance({ status: 'inactive' });
