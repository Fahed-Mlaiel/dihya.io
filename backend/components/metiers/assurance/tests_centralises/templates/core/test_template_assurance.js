...
const { renderassuranceTemplateUltra, validateassuranceTemplateUltra } = require('./template_assurance');

describe('Templates Core Central Tests', () => {
    it('should render a valid assurance template (nominal)', () => {
        const data = { id: '1', name: 'Cube', meta: { type: 'mesh' }, format: 'obj', i18n: { fr: 'Cube' } };
        const options = { audit: 'ok', accessibility: 'AA', rgpd: 'ok' };
        const result = renderassuranceTemplateUltra(data, options);
    ...
    it('should throw on missing id or name', () => {
        expect(() => renderassuranceTemplateUltra({ name: 'Cube' })).toThrow();
        expect(() => renderassuranceTemplateUltra({ id: '1' })).toThrow();
        expect(() => renderassuranceTemplateUltra({})).toThrow();
    });
    ...
    it('should validate assurance template (nominal, RGPD ok)', () => {
        const data = { id: '3', name: 'Pyramid', rgpd: 'ok' };
        expect(validateassuranceTemplateUltra(data)).toBe(true);
    });

    it('should invalidate assurance template (missing id/name or RGPD)', () => {
        expect(validateassuranceTemplateUltra({ name: 'NoId' })).toBe(false);
        expect(validateassuranceTemplateUltra({ id: '4' })).toBe(false);
        expect(validateassuranceTemplateUltra({ id: '5', name: 'Bad', rgpd: 'ko' })).toBe(false);
        expect(validateassuranceTemplateUltra(null)).toBe(false);
    });
