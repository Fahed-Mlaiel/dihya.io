/**
 * @file test_service_assurance.js
 * @description Tests ultra avancés clé en main pour ServiceAssurance (JS)
...
/**
 * Initialisation avancée du service métier assurance pour chaque test.
 */
describe('ServiceAssurance (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceAssurance({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getassuranceModel, listassuranceModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_assurance');

  test('getassuranceModel retourne un modèle valide', () => {
    const model = serviceModule.getassuranceModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getassuranceModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getassuranceModel()).toThrow('id requis');
  });

  test('listassuranceModels retourne une liste de modèles', () => {
    const models = serviceModule.listassuranceModels();
