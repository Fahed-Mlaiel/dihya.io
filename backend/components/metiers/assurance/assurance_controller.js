"use strict";
/**
 * @file assurance_controller.js
 * @module backend/components/metiers/assurance/assurance_controller
 * @description Contrôleur/service métier Assurance ultra avancé pour Dihya Coding.
...
import { anonymizeProject, exportProjects, getUserLang, i18n, validateAssuranceProject } from '../../../utils/validators';

/**
 * @typedef {Object} AssuranceProject
...
/**
 * Liste paginée, filtrée, multilingue des projets assurance
 * @param {object} req - Requête Express
 * @param {object} res - Réponse Express
 */
export async function getAssuranceProjects(req, res) {
  ...
  result = await runPluginHook('assurance', 'beforeList', result, req);
...
/**
 * Crée un projet assurance (validation, audit, plugins, fallback IA, RGPD)
 * @param {object} req
 * @param {object} res
 */
export async function createAssuranceProject(req, res) {
  requireRole(req, ['admin', 'user']);
  requireTenant(req);
  const lang = getUserLang(req);
  const data = validateAssuranceProject(req.body, lang);
  const project = {
    id: uuidv4(),
    ...data,
    owner: req.user.id,
  ...
  await runPluginHook('assurance', 'beforeCreate', project, req);
...
/**
 * Met à jour un projet assurance (validation, audit, plugins, RGPD)
 * @param {object} req
 * @param {object} res
 */
export async function updateAssuranceProject(req, res) {
  ...
  await runPluginHook('assurance', 'beforeUpdate', projects[idx], req);
...
/**
 * Supprime un projet assurance (audit, plugins, RGPD)
 * @param {object} req
 * @param {object} res
 */
export async function deleteAssuranceProject(req, res) {
  requireRole(req, ['admin']);
  requireTenant(req);
  const lang = getUserLang(req);
  const idx = projects.findIndex(p => p.id === req.params.id && p.tenant === req.user.tenant);
  ...
  await runPluginHook('assurance', 'beforeDelete', projects[idx], req);
...
/**
 * Export des projets assurance (RGPD, audit, multilingue)
 * @param {object} req
 * @param {object} res
 */
export async function exportAssuranceProjects(req, res) {
  requireRole(req, ['admin']);
  requireTenant(req);
  const lang = getUserLang(req);
...
/**
 * Purge des logs assurance (droit à l’oubli RGPD)
 * @param {object} req
 * @param {object} res
 */
export async function purgeAssuranceLogs(req, res) {
  requireRole(req, ['admin']);
  requireTenant(req);
  await purgeLogs('assurance', req.user.tenant);
  res.json({
...
/**
 * Résolveur GraphQL pour la création de projet assurance
 * @param {object} args
 * @param {object} context
 * @returns {AssuranceProject}
 */
export async function graphqlCreateAssuranceProject(args, context) {
  context.req.body = args.input;
  context.req.user = context.user;
  await createAssuranceProject(context.req, context.res);
}
