"""
service_automobile.py - Service principal automobile (Python)
...
def get_automobile_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleautomobile_{model_id}", "data": "..."}


def list_automobile_models():
    return [get_automobile_model(i) for i in range(1, 4)]
