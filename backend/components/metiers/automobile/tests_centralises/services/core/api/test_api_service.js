 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceAutomobile (JS)
...
function createApp() {
  ...
  app.post('/api/automobile/init', (req, res) => {
    try {
  ...
  app.post('/api/automobile/process', (req, res) => {
    try {
  ...
  app.get('/api/automobile/audit', (req, res) => {
    res.status(200).json(service.getAuditTrail());
  });

  // Route sécurité/role
  app.post('/api/automobile/secure', (req, res) => {
...
describe('API ServiceAutomobile (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/automobile/init initialise le service', async () => {
    const res = await request(app)
      .post('/api/automobile/init')
      .send({ config: { mode: 'ultra', secure: true } });
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
  });

  test('POST /api/automobile/process exécute une opération', async () => {
    await request(app).post('/api/automobile/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/automobile/process')
  ...
  test('POST /api/automobile/process refuse une opération invalide', async () => {
    await request(app).post('/api/automobile/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/automobile/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/automobile/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/automobile/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/automobile/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/automobile/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/automobile/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/automobile/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/automobile/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/automobile/secure')
