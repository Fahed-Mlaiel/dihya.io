/**
 * @file test_service_automobile.js
 * @description Tests ultra avancés clé en main pour ServiceAutomobile (JS)
...
/**
 * Initialisation avancée du service métier automobile pour chaque test.
 */
describe('ServiceAutomobile (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceAutomobile({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getautomobileModel, listautomobileModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_automobile');

  test('getautomobileModel retourne un modèle valide', () => {
    const model = serviceModule.getautomobileModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getautomobileModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getautomobileModel()).toThrow('id requis');
  });

  test('listautomobileModels retourne une liste de modèles', () => {
    const models = serviceModule.listautomobileModels();
