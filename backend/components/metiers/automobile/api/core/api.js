// api.js – Point d’entrée Express ultra avancé pour l’API Automobile
const express = require('express');
const router = express.Router();
const AutomobileController = require('../core/controllers/automobile_controller');
...
// GET automobile entity by ID
router.get('/automobile/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await AutomobileController.getById(req.params.id);
...
// POST create automobile entity
router.post('/automobile', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await AutomobileController.create(req.body);
...
// PUT update automobile entity
router.put('/automobile/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await AutomobileController.update(req.params.id, req.body);
...
// DELETE automobile entity
router.delete('/automobile/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await AutomobileController.delete(req.params.id);
