"""
service_arts.py - Service principal arts (Python)
...
def get_arts_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlearts_{model_id}", "data": "..."}


def list_arts_models():
    return [get_arts_model(i) for i in range(1, 4)]
