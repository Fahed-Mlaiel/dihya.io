...
describe('Views Router Central Tests', () => {
    ...
    it('GET /arts retourne une liste vide', async () => {
        const res = await request(app).get('/arts');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /arts crée un objet arts avec valeurs par défaut', async () => {
        const res = await request(app).post('/arts').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /arts crée un objet arts avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/arts').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /arts RGPD: ne retourne pas de données sensibles', async () => {
