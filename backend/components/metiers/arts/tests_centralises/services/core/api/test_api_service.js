 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceArts (JS)
...
function createApp() {
  ...
  app.post('/api/arts/init', (req, res) => {
    try {
  ...
  app.post('/api/arts/process', (req, res) => {
    try {
  ...
  app.get('/api/arts/audit', (req, res) => {
    res.status(200).json(service.getAuditTrail());
  });

  // Route sécurité/role
  app.post('/api/arts/secure', (req, res) => {
...
describe('API ServiceArts (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/arts/init initialise le service', async () => {
  ...
  test('POST /api/arts/process exécute une opération', async () => {
    await request(app).post('/api/arts/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/arts/process')
  ...
  test('POST /api/arts/process refuse une opération invalide', async () => {
    await request(app).post('/api/arts/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/arts/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/arts/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/arts/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/arts/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/arts/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/arts/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/arts/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/arts/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/arts/secure')
