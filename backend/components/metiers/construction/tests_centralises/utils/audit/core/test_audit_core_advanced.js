const { auditConstruction } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditConstruction({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditConstruction({ status: 'inactive' });
