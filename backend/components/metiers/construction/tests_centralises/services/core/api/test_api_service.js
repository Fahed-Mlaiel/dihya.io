 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceConstruction (JS)
...
function createApp() {
  ...
  app.post('/api/construction/init', (req, res) => {
    try {
  ...
  app.post('/api/construction/process', (req, res) => {
    try {
  ...
  app.get('/api/construction/audit', (req, res) => {
    res.status(200).json(service.getAuditTrail());
  });

  // Route sécurité/role
  app.post('/api/construction/secure', (req, res) => {
...
describe('API ServiceConstruction (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/construction/init initialise le service', async () => {
    const res = await request(app)
      .post('/api/construction/init')
      .send({ config: { mode: 'ultra', secure: true } });
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
  });

  test('POST /api/construction/process exécute une opération', async () => {
    await request(app).post('/api/construction/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/construction/process')
  ...
  test('POST /api/construction/process refuse une opération invalide', async () => {
    await request(app).post('/api/construction/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/construction/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/construction/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/construction/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/construction/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/construction/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/construction/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/construction/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/construction/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/construction/secure')
