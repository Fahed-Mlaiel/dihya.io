/**
 * @file test_service_construction.js
 * @description Tests ultra avancés clé en main pour ServiceConstruction (JS)
...
/**
 * Initialisation avancée du service métier construction pour chaque test.
 */
describe('ServiceConstruction (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceConstruction({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getconstructionModel, listconstructionModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_construction');

  test('getconstructionModel retourne un modèle valide', () => {
    const model = serviceModule.getconstructionModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getconstructionModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getconstructionModel()).toThrow('id requis');
  });

  test('listconstructionModels retourne une liste de modèles', () => {
    const models = serviceModule.listconstructionModels();
