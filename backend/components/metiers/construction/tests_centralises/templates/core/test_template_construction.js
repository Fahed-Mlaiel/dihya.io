...
const { renderconstructionTemplateUltra, validateconstructionTemplateUltra } = require('./template_construction');

describe('Templates Core Central Tests', () => {
    it('should render a valid construction template (nominal)', () => {
        const data = { id: '1', name: 'Cube', meta: { type: 'mesh' }, format: 'obj', i18n: { fr: 'Cube' } };
        const options = { audit: 'ok', accessibility: 'AA', rgpd: 'ok' };
        const result = renderconstructionTemplateUltra(data, options);
    ...
    it('should throw on missing id or name', () => {
        expect(() => renderconstructionTemplateUltra({ name: 'Cube' })).toThrow();
        expect(() => renderconstructionTemplateUltra({ id: '1' })).toThrow();
        expect(() => renderconstructionTemplateUltra({})).toThrow();
    });
    ...
    it('should validate construction template (nominal, RGPD ok)', () => {
        const data = { id: '3', name: 'Pyramid', rgpd: 'ok' };
        expect(validateconstructionTemplateUltra(data)).toBe(true);
    });

    it('should invalidate construction template (missing id/name or RGPD)', () => {
        expect(validateconstructionTemplateUltra({ name: 'NoId' })).toBe(false);
        expect(validateconstructionTemplateUltra({ id: '4' })).toBe(false);
        expect(validateconstructionTemplateUltra({ id: '5', name: 'Bad', rgpd: 'ko' })).toBe(false);
        expect(validateconstructionTemplateUltra(null)).toBe(false);
    });
