// api.js – Point d’entrée Express ultra avancé pour l’API Construction
const express = require('express');
const router = express.Router();
const ConstructionController = require('../core/controllers/construction_controller');
...
// GET construction entity by ID
router.get('/construction/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await ConstructionController.getById(req.params.id);
...
// POST create construction entity
router.post('/construction', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await ConstructionController.create(req.body);
...
// PUT update construction entity
router.put('/construction/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await ConstructionController.update(req.params.id, req.body);
