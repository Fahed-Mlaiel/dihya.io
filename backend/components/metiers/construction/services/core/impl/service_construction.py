"""
service_construction.py - Service principal construction (Python)
...
def get_construction_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleconstruction_{model_id}", "data": "..."}


def list_construction_models():
    return [get_construction_model(i) for i in range(1, 4)]
