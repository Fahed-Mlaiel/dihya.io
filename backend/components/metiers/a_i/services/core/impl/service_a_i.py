"""
service_a_i.py - Service principal a_i (Python)
...
def get_a_i_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlea_i_{model_id}", "data": "..."}


def list_a_i_models():
    return [get_a_i_model(i) for i in range(1, 4)]
