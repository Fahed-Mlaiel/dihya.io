// api.js – Point d’entrée Express ultra avancé pour l’API A_I
const express = require('express');
const router = express.Router();
const A_IController = require('../core/controllers/a_i_controller');
...
// GET a_i entity by ID
router.get('/a_i/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await A_IController.getById(req.params.id);
...
// POST create a_i entity
router.post('/a_i', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await A_IController.create(req.body);
...
// PUT update a_i entity
router.put('/a_i/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await A_IController.update(req.params.id, req.body);
...
// DELETE a_i entity
router.delete('/a_i/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await A_IController.delete(req.params.id);
