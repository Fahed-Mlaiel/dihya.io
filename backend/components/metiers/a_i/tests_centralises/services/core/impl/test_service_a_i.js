/**
 * @file test_service_a_i.js
 * @description Tests ultra avancés clé en main pour ServiceA_I (JS)
...
/**
 * Initialisation avancée du service métier a_i pour chaque test.
 */
describe('ServiceA_I (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceA_I({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (geta_iModel, lista_iModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_a_i');

  test('geta_iModel retourne un modèle valide', () => {
    const model = serviceModule.geta_iModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('geta_iModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.geta_iModel()).toThrow('id requis');
  });

  test('lista_iModels retourne une liste de modèles', () => {
    const models = serviceModule.lista_iModels();
