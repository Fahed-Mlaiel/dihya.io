...
describe('Views Router Central Tests', () => {
    ...
    it('GET /a_i retourne une liste vide', async () => {
        const res = await request(app).get('/a_i');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /a_i crée un objet a_i avec valeurs par défaut', async () => {
        const res = await request(app).post('/a_i').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /a_i crée un objet a_i avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/a_i').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /a_i RGPD: ne retourne pas de données sensibles', async () => {
