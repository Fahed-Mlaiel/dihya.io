"""
service_environnement.py - Service principal environnement (Python)
...
def get_environnement_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleenvironnement_{model_id}", "data": "..."}


def list_environnement_models():
    return [get_environnement_model(i) for i in range(1, 4)]
