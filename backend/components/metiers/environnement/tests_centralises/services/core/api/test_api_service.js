 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceEnvironnement (JS)
...
function createApp() {
  ...
  app.post('/api/environnement/init', (req, res) => {
    try {
  ...
  app.post('/api/environnement/process', (req, res) => {
    try {
  ...
  app.get('/api/environnement/audit', (req, res) => {
    res.status(200).json(service.getAuditTrail());
  });

  // Route sécurité/role
  app.post('/api/environnement/secure', (req, res) => {
...
describe('API ServiceEnvironnement (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/environnement/init initialise le service', async () => {
    const res = await request(app)
      .post('/api/environnement/init')
      .send({ config: { mode: 'ultra', secure: true } });
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
  });

  test('POST /api/environnement/process exécute une opération', async () => {
    await request(app).post('/api/environnement/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/environnement/process')
  ...
  test('POST /api/environnement/process refuse une opération invalide', async () => {
    await request(app).post('/api/environnement/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/environnement/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/environnement/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/environnement/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/environnement/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/environnement/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/environnement/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/environnement/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/environnement/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/environnement/secure')
