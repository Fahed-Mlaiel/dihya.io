// api.js – Point d’entrée Express ultra avancé pour l’API Environnement
const express = require('express');
const router = express.Router();
const EnvironnementController = require('../core/controllers/environnement_controller');
...
// GET environnement entity by ID
router.get('/environnement/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await EnvironnementController.getById(req.params.id);
...
// POST create environnement entity
router.post('/environnement', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await EnvironnementController.create(req.body);
...
// PUT update environnement entity
router.put('/environnement/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await EnvironnementController.update(req.params.id, req.body);
...
// DELETE environnement entity
router.delete('/environnement/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await EnvironnementController.delete(req.params.id);
