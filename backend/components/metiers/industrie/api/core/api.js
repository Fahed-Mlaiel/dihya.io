// api.js – Point d’entrée Express ultra avancé pour l’API Industrie
const express = require('express');
const router = express.Router();
const IndustrieController = require('../core/controllers/industrie_controller');
...
// GET industrie entity by ID
router.get('/industrie/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await IndustrieController.getById(req.params.id);
...
// POST create industrie entity
router.post('/industrie', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await IndustrieController.create(req.body);
...
// PUT update industrie entity
router.put('/industrie/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await IndustrieController.update(req.params.id, req.body);
...
// DELETE industrie entity
router.delete('/industrie/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await IndustrieController.delete(req.params.id);
