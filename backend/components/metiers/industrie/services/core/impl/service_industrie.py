"""
service_industrie.py - Service principal industrie (Python)
...
def get_industrie_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleindustrie_{model_id}", "data": "..."}


def list_industrie_models():
    return [get_industrie_model(i) for i in range(1, 4)]
