"""
service_vr_ar.py - Service principal vr_ar (Python)
...
def get_vr_ar_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlevr_ar_{model_id}", "data": "..."}


def list_vr_ar_models():
    return [get_vr_ar_model(i) for i in range(1, 4)]
