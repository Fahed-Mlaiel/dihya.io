...
const { rendervr_arTemplateUltra, validatevr_arTemplateUltra } = require('./template_vr_ar');

describe('Templates Core Central Tests', () => {
    it('should render a valid vr_ar template (nominal)', () => {
        const data = { id: '1', name: 'Cube', meta: { type: 'mesh' }, format: 'obj', i18n: { fr: 'Cube' } };
        const options = { audit: 'ok', accessibility: 'AA', rgpd: 'ok' };
        const result = rendervr_arTemplateUltra(data, options);
    ...
    it('should throw on missing id or name', () => {
        expect(() => rendervr_arTemplateUltra({ name: 'Cube' })).toThrow();
        expect(() => rendervr_arTemplateUltra({ id: '1' })).toThrow();
        expect(() => rendervr_arTemplateUltra({})).toThrow();
    });
    ...
    it('should validate vr_ar template (nominal, RGPD ok)', () => {
        const data = { id: '3', name: 'Pyramid', rgpd: 'ok' };
        expect(validatevr_arTemplateUltra(data)).toBe(true);
    });

    it('should invalidate vr_ar template (missing id/name or RGPD)', () => {
        expect(validatevr_arTemplateUltra({ name: 'NoId' })).toBe(false);
        expect(validatevr_arTemplateUltra({ id: '4' })).toBe(false);
        expect(validatevr_arTemplateUltra({ id: '5', name: 'Bad', rgpd: 'ko' })).toBe(false);
        expect(validatevr_arTemplateUltra(null)).toBe(false);
    });
