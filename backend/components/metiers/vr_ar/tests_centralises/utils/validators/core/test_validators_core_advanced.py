...
import pytest
from backend.components.metiers.vr_ar.utils.validators.core import validators

def test_validate_vr_ar_nominal():
    data = {"nom": "Projet X", "statut": "actif"}
    assert validators.validate_vr_ar(data) is True

def test_validate_vr_ar_missing_nom():
    data = {"statut": "actif"}
    with pytest.raises(ValueError, match="Nom requis"):
        validators.validate_vr_ar(data)

def test_validate_vr_ar_invalid_statut():
    data = {"nom": "Projet Y", "statut": "supprimé"}
    with pytest.raises(ValueError, match="Statut invalide"):
        validators.validate_vr_ar(data)

def test_validate_vr_ar_statut_none():
    data = {"nom": "Projet Z", "statut": None}
    assert validators.validate_vr_ar(data) is True

def test_validate_vr_ar_statut_archived():
    data = {"nom": "Projet A", "statut": "archivé"}
    assert validators.validate_vr_ar(data) is True

def test_validate_vr_ar_extra_fields():
