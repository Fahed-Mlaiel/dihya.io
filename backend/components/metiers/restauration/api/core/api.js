// api.js – Point d’entrée Express ultra avancé pour l’API RestauratioN
const express = require('express');
const router = express.Router();
const RestauratioNController = require('../core/controllers/restauration_controller');
...
// GET restauration entity by ID
router.get('/restauration/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await RestauratioNController.getById(req.params.id);
...
// POST create restauration entity
router.post('/restauration', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await RestauratioNController.create(req.body);
...
// PUT update restauration entity
router.put('/restauration/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await RestauratioNController.update(req.params.id, req.body);
...
// DELETE restauration entity
router.delete('/restauration/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await RestauratioNController.delete(req.params.id);
