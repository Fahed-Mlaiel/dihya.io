"""
service_restauration.py - Service principal restauration (Python)
...
def get_restauration_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlerestauration_{model_id}", "data": "..."}


def list_restauration_models():
    return [get_restauration_model(i) for i in range(1, 4)]
