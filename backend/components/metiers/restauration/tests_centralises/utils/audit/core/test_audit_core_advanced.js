const { auditRestauratioN } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditRestauratioN({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditRestauratioN({ status: 'inactive' });
