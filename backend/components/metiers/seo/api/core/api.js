// api.js – Point d’entrée Express ultra avancé pour l’API Seo
const express = require('express');
const router = express.Router();
const SeoController = require('../core/controllers/seo_controller');
...
// GET seo entity by ID
router.get('/seo/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await SeoController.getById(req.params.id);
...
// POST create seo entity
router.post('/seo', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await SeoController.create(req.body);
...
// PUT update seo entity
router.put('/seo/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await SeoController.update(req.params.id, req.body);
...
router.delete('/seo/:id', async (req, res) => {
  ...
  try {
    await SeoController.delete(req.params.id);
