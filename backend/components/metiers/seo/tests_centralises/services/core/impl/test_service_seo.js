/**
 * @file test_service_seo.js
 * @description Tests ultra avancés clé en main pour ServiceSeo (JS)
...
/**
 * Initialisation avancée du service métier seo pour chaque test.
 */
describe('ServiceSeo (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceSeo({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getseoModel, listseoModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_seo');

  test('getseoModel retourne un modèle valide', () => {
    const model = serviceModule.getseoModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getseoModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getseoModel()).toThrow('id requis');
  });

  test('listseoModels retourne une liste de modèles', () => {
    const models = serviceModule.listseoModels();
