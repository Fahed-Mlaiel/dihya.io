...
import pytest
from backend.components.metiers.seo.utils.validators.core import validators

def test_validate_seo_nominal():
    data = {"nom": "Projet X", "statut": "actif"}
    assert validators.validate_seo(data) is True

def test_validate_seo_missing_nom():
    data = {"statut": "actif"}
    with pytest.raises(ValueError, match="Nom requis"):
        validators.validate_seo(data)

def test_validate_seo_invalid_statut():
    data = {"nom": "Projet Y", "statut": "supprimé"}
    with pytest.raises(ValueError, match="Statut invalide"):
        validators.validate_seo(data)

def test_validate_seo_statut_none():
    data = {"nom": "Projet Z", "statut": None}
    assert validators.validate_seo(data) is True

def test_validate_seo_statut_archived():
    data = {"nom": "Projet A", "statut": "archivé"}
    assert validators.validate_seo(data) is True

def test_validate_seo_extra_fields():
