"""
service_seo.py - Service principal seo (Python)
...
def get_seo_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleseo_{model_id}", "data": "..."}


def list_seo_models():
    return [get_seo_model(i) for i in range(1, 4)]
