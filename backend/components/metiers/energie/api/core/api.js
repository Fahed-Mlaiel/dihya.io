// api.js – Point d’entrée Express ultra avancé pour l’API Energie
const express = require('express');
const router = express.Router();
const EnergieController = require('../core/controllers/energie_controller');
...
// GET energie entity by ID
router.get('/energie/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await EnergieController.getById(req.params.id);
...
// POST create energie entity
router.post('/energie', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await EnergieController.create(req.body);
...
// PUT update energie entity
router.put('/energie/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await EnergieController.update(req.params.id, req.body);
...
// DELETE energie entity
router.delete('/energie/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await EnergieController.delete(req.params.id);
