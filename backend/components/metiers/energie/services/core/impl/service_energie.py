"""
service_energie.py - Service principal energie (Python)
...
def get_energie_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleenergie_{model_id}", "data": "..."}


def list_energie_models():
    return [get_energie_model(i) for i in range(1, 4)]
