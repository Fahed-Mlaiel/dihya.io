...
describe('Guide Exemple Legacy - JS', () => {
  it('doit valider le guide exemple nominal', () => {
    expect(guideExemple()).toBe('Exemple de guide d’audit legacy Energie (JS)');
  });
  it('doit gérer les cas limites et erreurs', () => {
    ...
    expect(guideExemple.call(null)).toBe('Exemple de guide d’audit legacy Energie (JS)');
    expect(guideExemple.apply({}, [])).toBe('Exemple de guide d’audit legacy Energie (JS)');
  });
  it('doit être conforme accessibilité/audit', () => {
    // Vérifie que le texte est bien lisible et conforme
    const result = guideExemple();
    expect(result).toMatch(/audit legacy Energie/);
