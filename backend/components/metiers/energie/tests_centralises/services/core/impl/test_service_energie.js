/**
 * @file test_service_energie.js
 * @description Tests ultra avancés clé en main pour ServiceEnergie (JS)
...
/**
 * Initialisation avancée du service métier energie pour chaque test.
 */
describe('ServiceEnergie (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceEnergie({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getenergieModel, listenergieModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_energie');

  test('getenergieModel retourne un modèle valide', () => {
    const model = serviceModule.getenergieModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getenergieModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getenergieModel()).toThrow('id requis');
  });

  test('listenergieModels retourne une liste de modèles', () => {
    const models = serviceModule.listenergieModels();
