"""
service_transport.py - Service principal transport (Python)
...
def get_transport_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèletransport_{model_id}", "data": "..."}


def list_transport_models():
    return [get_transport_model(i) for i in range(1, 4)]
