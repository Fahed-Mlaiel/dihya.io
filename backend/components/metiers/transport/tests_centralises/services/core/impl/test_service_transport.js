/**
 * @file test_service_transport.js
 * @description Tests ultra avancés clé en main pour ServiceTransport (JS)
...
/**
 * Initialisation avancée du service métier transport pour chaque test.
 */
describe('ServiceTransport (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceTransport({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (gettransportModel, listtransportModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_transport');

  test('gettransportModel retourne un modèle valide', () => {
    const model = serviceModule.gettransportModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('gettransportModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.gettransportModel()).toThrow('id requis');
  });

  test('listtransportModels retourne une liste de modèles', () => {
    const models = serviceModule.listtransportModels();
