const { auditTransport } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditTransport({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditTransport({ status: 'inactive' });
