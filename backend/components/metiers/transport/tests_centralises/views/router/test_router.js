...
describe('Views Router Central Tests', () => {
    ...
    it('GET /transport retourne une liste vide', async () => {
        const res = await request(app).get('/transport');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /transport crée un objet transport avec valeurs par défaut', async () => {
        const res = await request(app).post('/transport').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /transport crée un objet transport avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/transport').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /transport RGPD: ne retourne pas de données sensibles', async () => {
