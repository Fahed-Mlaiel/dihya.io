// api.js – Point d’entrée Express ultra avancé pour l’API Immobilier
const express = require('express');
const router = express.Router();
const ImmobilierController = require('../core/controllers/immobilier_controller');
...
// GET immobilier entity by ID
router.get('/immobilier/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await ImmobilierController.getById(req.params.id);
...
// POST create immobilier entity
router.post('/immobilier', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await ImmobilierController.create(req.body);
...
// PUT update immobilier entity
router.put('/immobilier/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await ImmobilierController.update(req.params.id, req.body);
...
// DELETE immobilier entity
router.delete('/immobilier/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await ImmobilierController.delete(req.params.id);
