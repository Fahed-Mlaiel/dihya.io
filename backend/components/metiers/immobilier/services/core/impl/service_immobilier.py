"""
service_immobilier.py - Service principal immobilier (Python)
...
def get_immobilier_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleimmobilier_{model_id}", "data": "..."}


def list_immobilier_models():
    return [get_immobilier_model(i) for i in range(1, 4)]
