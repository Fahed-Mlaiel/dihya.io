/**
 * @file test_service_immobilier.js
 * @description Tests ultra avancés clé en main pour ServiceImmobilier (JS)
...
/**
 * Initialisation avancée du service métier immobilier pour chaque test.
 */
describe('ServiceImmobilier (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceImmobilier({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getimmobilierModel, listimmobilierModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_immobilier');

  test('getimmobilierModel retourne un modèle valide', () => {
    const model = serviceModule.getimmobilierModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getimmobilierModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getimmobilierModel()).toThrow('id requis');
  });

  test('listimmobilierModels retourne une liste de modèles', () => {
    const models = serviceModule.listimmobilierModels();
