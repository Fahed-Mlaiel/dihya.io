 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceImmobilier (JS)
...
function createApp() {
  ...
  app.post('/api/immobilier/init', (req, res) => {
    try {
  ...
  app.post('/api/immobilier/process', (req, res) => {
    try {
  ...
  app.get('/api/immobilier/audit', (req, res) => {
    res.status(200).json(service.getAuditTrail());
  });

  // Route sécurité/role
  app.post('/api/immobilier/secure', (req, res) => {
...
describe('API ServiceImmobilier (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/immobilier/init initialise le service', async () => {
    const res = await request(app)
      .post('/api/immobilier/init')
      .send({ config: { mode: 'ultra', secure: true } });
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
  });

  test('POST /api/immobilier/process exécute une opération', async () => {
    await request(app).post('/api/immobilier/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/immobilier/process')
  ...
  test('POST /api/immobilier/process refuse une opération invalide', async () => {
    await request(app).post('/api/immobilier/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/immobilier/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/immobilier/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/immobilier/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/immobilier/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/immobilier/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/immobilier/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/immobilier/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/immobilier/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/immobilier/secure')
