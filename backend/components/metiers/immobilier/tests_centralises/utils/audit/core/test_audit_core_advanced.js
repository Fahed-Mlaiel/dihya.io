const { auditImmobilier } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditImmobilier({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditImmobilier({ status: 'inactive' });
