"""
service_beaute.py - Service principal beaute (Python)
...
def get_beaute_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlebeaute_{model_id}", "data": "..."}


def list_beaute_models():
    return [get_beaute_model(i) for i in range(1, 4)]
