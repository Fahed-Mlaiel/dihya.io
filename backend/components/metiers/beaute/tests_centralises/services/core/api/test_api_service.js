 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceBeaute (JS)
...
function createApp() {
  ...
  app.post('/api/beaute/init', (req, res) => {
    try {
  ...
  app.post('/api/beaute/process', (req, res) => {
    try {
  ...
  app.get('/api/beaute/audit', (req, res) => {
    res.status(200).json(service.getAuditTrail());
  });

  // Route sécurité/role
  app.post('/api/beaute/secure', (req, res) => {
...
describe('API ServiceBeaute (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/beaute/init initialise le service', async () => {
    const res = await request(app)
      .post('/api/beaute/init')
      .send({ config: { mode: 'ultra', secure: true } });
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
  });

  test('POST /api/beaute/process exécute une opération', async () => {
    await request(app).post('/api/beaute/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/beaute/process')
  ...
  test('POST /api/beaute/process refuse une opération invalide', async () => {
    await request(app).post('/api/beaute/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/beaute/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/beaute/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/beaute/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/beaute/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/beaute/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/beaute/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/beaute/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/beaute/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/beaute/secure')
