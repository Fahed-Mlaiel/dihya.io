/**
 * @file test_service_beaute.js
 * @description Tests ultra avancés clé en main pour ServiceBeaute (JS)
...
/**
 * Initialisation avancée du service métier beaute pour chaque test.
 */
describe('ServiceBeaute (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceBeaute({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getbeauteModel, listbeauteModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_beaute');

  test('getbeauteModel retourne un modèle valide', () => {
    const model = serviceModule.getbeauteModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getbeauteModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getbeauteModel()).toThrow('id requis');
  });

  test('listbeauteModels retourne une liste de modèles', () => {
    const models = serviceModule.listbeauteModels();
