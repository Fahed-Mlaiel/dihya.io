...
import pytest
from backend.components.metiers.beaute.utils.validators.core import validators

def test_validate_beaute_nominal():
    data = {"nom": "Projet X", "statut": "actif"}
    assert validators.validate_beaute(data) is True

def test_validate_beaute_missing_nom():
    data = {"statut": "actif"}
    with pytest.raises(ValueError, match="Nom requis"):
        validators.validate_beaute(data)

def test_validate_beaute_invalid_statut():
    data = {"nom": "Projet Y", "statut": "supprimé"}
    with pytest.raises(ValueError, match="Statut invalide"):
        validators.validate_beaute(data)

def test_validate_beaute_statut_none():
    data = {"nom": "Projet Z", "statut": None}
    assert validators.validate_beaute(data) is True

def test_validate_beaute_statut_archived():
    data = {"nom": "Projet A", "statut": "archivé"}
    assert validators.validate_beaute(data) is True

def test_validate_beaute_extra_fields():
