"""
health_controller.py – Contrôleur ultra avancé API Health (Python)
...
from ..validators.validators import validate_health_entity
...
def get_health(id: int):
    ...
    entity = db_find_by_id("health", id)
    if not entity:
        return None
...
@router.post("/health")
def create_health(data: dict):
    before_action("create", data)
    validate_health_entity(data)
    created = db_insert("health", data)
...
def update_health(id: int, data: dict):
    before_action("update", {"id": id, **data})
    validate_health_entity(data)
    updated = db_update("health", id, data)
...
def delete_health(id: int):
    before_action("delete", {"id": id})
    deleted = db_delete("health", id)
    audit_entity({"id": id}, "delete")
    after_action("delete", {"id": id})
    return deleted


# Es gibt keine Klasse HealthController, sondern nur Funktions-Exporte.
...
class HealthController:
    ...
    @staticmethod
    def create(data):
        return create_health(data)

    @staticmethod
    def update(id, data):
        return update_health(id, data)

    @staticmethod
    def delete(id):
        return delete_health(id)
