// api.js – Point d’entrée Express ultra avancé pour l’API Health
const express = require('express');
const router = express.Router();
const HealthController = require('../core/controllers/health_controller');
...
// GET health entity by ID
router.get('/health/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await HealthController.getById(req.params.id);
...
// POST create health entity
router.post('/health', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await HealthController.create(req.body);
...
// PUT update health entity
router.put('/health/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await HealthController.update(req.params.id, req.body);
...
router.delete('/health/:id', async (req, res) => {
  ...
  try {
    await HealthController.delete(req.params.id);
