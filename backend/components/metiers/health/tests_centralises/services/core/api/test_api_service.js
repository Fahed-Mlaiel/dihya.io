 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceHealth (JS)
...
function createApp() {
  ...
  app.post('/api/health/init', (req, res) => {
    try {
  ...
  app.post('/api/health/process', (req, res) => {
    try {
  ...
  // Route sécurité/role
  app.post('/api/health/secure', (req, res) => {
...
describe('API ServiceHealth (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/health/init initialise le service', async () => {
  ...
  test('POST /api/health/process exécute une opération', async () => {
    await request(app).post('/api/health/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
  ...
  test('POST /api/health/process refuse une opération invalide', async () => {
    await request(app).post('/api/health/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/health/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/health/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/health/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/health/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/health/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/health/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/health/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/health/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/health/secure')
