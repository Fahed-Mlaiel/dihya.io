/**
 * @file test_service_health.js
 * @description Tests ultra avancés clé en main pour ServiceHealth (JS)
...
/**
 * Initialisation avancée du service métier health pour chaque test.
 */
describe('ServiceHealth (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceHealth({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (gethealthModel, listhealthModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_health');

  test('gethealthModel retourne un modèle valide', () => {
    const model = serviceModule.gethealthModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('gethealthModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.gethealthModel()).toThrow('id requis');
  });

  test('listhealthModels retourne une liste de modèles', () => {
    const models = serviceModule.listhealthModels();
