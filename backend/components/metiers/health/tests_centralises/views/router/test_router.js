...
describe('Views Router Central Tests', () => {
    ...
    it('GET /health retourne une liste vide', async () => {
        const res = await request(app).get('/health');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /health crée un objet health avec valeurs par défaut', async () => {
        const res = await request(app).post('/health').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /health crée un objet health avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/health').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /health RGPD: ne retourne pas de données sensibles', async () => {
