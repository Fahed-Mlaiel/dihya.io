"""
service_health.py - Service principal health (Python)
...
def get_health_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlehealth_{model_id}", "data": "..."}


def list_health_models():
    return [get_health_model(i) for i in range(1, 4)]
