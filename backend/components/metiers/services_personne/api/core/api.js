// api.js – Point d’entrée Express ultra avancé pour l’API ServicesPersonne
const express = require('express');
const router = express.Router();
const ServicesPersonneController = require('../core/controllers/services_personne_controller');
...
// GET services_personne entity by ID
router.get('/services_personne/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await ServicesPersonneController.getById(req.params.id);
...
// POST create services_personne entity
router.post('/services_personne', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await ServicesPersonneController.create(req.body);
...
// PUT update services_personne entity
router.put('/services_personne/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await ServicesPersonneController.update(req.params.id, req.body);
...
// DELETE services_personne entity
router.delete('/services_personne/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await ServicesPersonneController.delete(req.params.id);
