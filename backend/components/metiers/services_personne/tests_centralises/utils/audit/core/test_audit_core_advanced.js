const { auditServicesPersonne } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditServicesPersonne({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditServicesPersonne({ status: 'inactive' });
  ...
  it('gère les cas limites (status manquant)', () => {
    const res = auditServicesPersonne({});
