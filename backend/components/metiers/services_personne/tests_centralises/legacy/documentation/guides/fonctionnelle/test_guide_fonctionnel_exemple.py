...
def test_guide_fonctionnel_exemple_output(capsys):
    from backend.components.metiers.services_personne.legacy.documentation.guides.fonctionnelle import (
        guide_fonctionnel_exemple,
    )

    guide_fonctionnel_exemple.guide_fonctionnel_exemple()
    captured = capsys.readouterr()
    assert "Guide fonctionnel legacy ServicesPersonne" in captured.out
