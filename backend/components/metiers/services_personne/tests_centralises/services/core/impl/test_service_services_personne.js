/**
 * @file test_service_services_personne.js
 * @description Tests ultra avancés clé en main pour ServiceServicesPersonne (JS)
...
/**
 * Initialisation avancée du service métier services_personne pour chaque test.
 */
describe('ServiceServicesPersonne (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceServicesPersonne({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getservices_personneModel, listservices_personneModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_services_personne');

  test('getservices_personneModel retourne un modèle valide', () => {
    const model = serviceModule.getservices_personneModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getservices_personneModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getservices_personneModel()).toThrow('id requis');
  });

  test('listservices_personneModels retourne une liste de modèles', () => {
    const models = serviceModule.listservices_personneModels();
