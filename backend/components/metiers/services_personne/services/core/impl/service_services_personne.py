"""
service_services_personne.py - Service principal services_personne (Python)
...
def get_services_personne_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleservices_personne_{model_id}", "data": "..."}


def list_services_personne_models():
    return [get_services_personne_model(i) for i in range(1, 4)]
