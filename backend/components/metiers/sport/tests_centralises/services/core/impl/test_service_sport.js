/**
 * @file test_service_sport.js
 * @description Tests ultra avancés clé en main pour ServiceSport (JS)
...
/**
 * Initialisation avancée du service métier sport pour chaque test.
 */
describe('ServiceSport (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceSport({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getsportModel, listsportModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_sport');

  test('getsportModel retourne un modèle valide', () => {
    const model = serviceModule.getsportModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getsportModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getsportModel()).toThrow('id requis');
  });

  test('listsportModels retourne une liste de modèles', () => {
    const models = serviceModule.listsportModels();
