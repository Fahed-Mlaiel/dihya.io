...
import pytest
from backend.components.metiers.sport.utils.validators.core import validators

def test_validate_sport_nominal():
    data = {"nom": "Projet X", "statut": "actif"}
    assert validators.validate_sport(data) is True

def test_validate_sport_missing_nom():
    data = {"statut": "actif"}
    with pytest.raises(ValueError, match="Nom requis"):
        validators.validate_sport(data)

def test_validate_sport_invalid_statut():
    data = {"nom": "Projet Y", "statut": "supprimé"}
    with pytest.raises(ValueError, match="Statut invalide"):
        validators.validate_sport(data)

def test_validate_sport_statut_none():
    data = {"nom": "Projet Z", "statut": None}
    assert validators.validate_sport(data) is True

def test_validate_sport_statut_archived():
    data = {"nom": "Projet A", "statut": "archivé"}
    assert validators.validate_sport(data) is True

def test_validate_sport_extra_fields():
