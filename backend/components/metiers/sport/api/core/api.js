// api.js – Point d’entrée Express ultra avancé pour l’API Sport
const express = require('express');
const router = express.Router();
const SportController = require('../core/controllers/sport_controller');
...
// GET sport entity by ID
router.get('/sport/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await SportController.getById(req.params.id);
...
// POST create sport entity
router.post('/sport', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await SportController.create(req.body);
...
// PUT update sport entity
router.put('/sport/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await SportController.update(req.params.id, req.body);
...
router.delete('/sport/:id', async (req, res) => {
  ...
  try {
    await SportController.delete(req.params.id);
