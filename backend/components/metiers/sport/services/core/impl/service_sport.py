"""
service_sport.py - Service principal sport (Python)
...
def get_sport_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlesport_{model_id}", "data": "..."}


def list_sport_models():
    return [get_sport_model(i) for i in range(1, 4)]
