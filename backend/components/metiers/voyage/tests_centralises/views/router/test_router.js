...
describe('Views Router Central Tests', () => {
    ...
    it('GET /voyage retourne une liste vide', async () => {
        const res = await request(app).get('/voyage');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /voyage crée un objet voyage avec valeurs par défaut', async () => {
        const res = await request(app).post('/voyage').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /voyage crée un objet voyage avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/voyage').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /voyage RGPD: ne retourne pas de données sensibles', async () => {
