"""
service_voyage.py - Service principal voyage (Python)
...
def get_voyage_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlevoyage_{model_id}", "data": "..."}


def list_voyage_models():
    return [get_voyage_model(i) for i in range(1, 4)]
