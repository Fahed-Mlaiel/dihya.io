// api.js – Point d’entrée Express ultra avancé pour l’API Recherche
const express = require('express');
const router = express.Router();
const RechercheController = require('../core/controllers/recherche_controller');
...
// GET recherche entity by ID
router.get('/recherche/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await RechercheController.getById(req.params.id);
...
// POST create recherche entity
router.post('/recherche', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await RechercheController.create(req.body);
...
// PUT update recherche entity
router.put('/recherche/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await RechercheController.update(req.params.id, req.body);
...
// DELETE recherche entity
router.delete('/recherche/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await RechercheController.delete(req.params.id);
