"""
service_recherche.py - Service principal recherche (Python)
...
def get_recherche_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlerecherche_{model_id}", "data": "..."}


def list_recherche_models():
    return [get_recherche_model(i) for i in range(1, 4)]
