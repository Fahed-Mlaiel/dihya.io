const { auditRecherche } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditRecherche({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditRecherche({ status: 'inactive' });
