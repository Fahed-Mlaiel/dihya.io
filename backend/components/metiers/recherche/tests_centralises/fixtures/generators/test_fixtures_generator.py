...
from recherche.fixtures.generators import fixtures_generator
...
def test_generate_model_nominal():
    from backend.components.metiers.recherche.fixtures.generators import fixtures_generator

    model = fixtures_generator.generate_model()
    assert model["id"].startswith("model-")
    assert model["name"] == "Modèle Test"
    assert model["type"] == "recherche"
