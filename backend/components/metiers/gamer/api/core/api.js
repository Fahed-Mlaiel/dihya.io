// api.js – Point d’entrée Express ultra avancé pour l’API Gamer
const express = require('express');
const router = express.Router();
const GamerController = require('../core/controllers/gamer_controller');
...
// GET gamer entity by ID
router.get('/gamer/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await GamerController.getById(req.params.id);
...
// POST create gamer entity
router.post('/gamer', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await GamerController.create(req.body);
...
// PUT update gamer entity
router.put('/gamer/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await GamerController.update(req.params.id, req.body);
...
// DELETE gamer entity
router.delete('/gamer/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await GamerController.delete(req.params.id);
