"""
service_gamer.py - Service principal gamer (Python)
...
def get_gamer_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlegamer_{model_id}", "data": "..."}


def list_gamer_models():
    return [get_gamer_model(i) for i in range(1, 4)]
