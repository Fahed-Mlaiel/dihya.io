/**
 * @file test_service_gamer.js
 * @description Tests ultra avancés clé en main pour ServiceGamer (JS)
...
/**
 * Initialisation avancée du service métier gamer pour chaque test.
 */
describe('ServiceGamer (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceGamer({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getgamerModel, listgamerModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_gamer');

  test('getgamerModel retourne un modèle valide', () => {
    const model = serviceModule.getgamerModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getgamerModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getgamerModel()).toThrow('id requis');
  });

  test('listgamerModels retourne une liste de modèles', () => {
    const models = serviceModule.listgamerModels();
