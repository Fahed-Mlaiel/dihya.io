...
describe('Views Router Central Tests', () => {
    ...
    it('GET /gamer retourne une liste vide', async () => {
        const res = await request(app).get('/gamer');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /gamer crée un objet gamer avec valeurs par défaut', async () => {
        const res = await request(app).post('/gamer').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /gamer crée un objet gamer avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/gamer').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /gamer RGPD: ne retourne pas de données sensibles', async () => {
