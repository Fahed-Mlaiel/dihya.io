const { auditadministration_publique } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditadministration_publique({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditadministration_publique({ status: 'inactive' });
  ...
  it('gère les cas limites (status manquant)', () => {
    const res = auditadministration_publique({});
