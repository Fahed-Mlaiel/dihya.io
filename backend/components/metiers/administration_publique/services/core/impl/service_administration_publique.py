"""
service_administration_publique.py - Service principal administration_publique (Python)
...
class Serviceadministration_publique:
...
def get_administration_publique_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleadministration_publique_{model_id}", "data": "..."}


def list_administration_publique_models():
    return [get_administration_publique_model(i) for i in range(1, 4)]
