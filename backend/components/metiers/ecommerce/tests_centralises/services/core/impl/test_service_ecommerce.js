/**
 * @file test_service_ecommerce.js
 * @description Tests ultra avancés clé en main pour ServiceEcommerce (JS)
...
/**
 * Initialisation avancée du service métier ecommerce pour chaque test.
 */
describe('ServiceEcommerce (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceEcommerce({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getecommerceModel, listecommerceModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_ecommerce');

  test('getecommerceModel retourne un modèle valide', () => {
    const model = serviceModule.getecommerceModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getecommerceModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getecommerceModel()).toThrow('id requis');
  });

  test('listecommerceModels retourne une liste de modèles', () => {
    const models = serviceModule.listecommerceModels();
