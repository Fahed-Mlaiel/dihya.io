 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceEcommerce (JS)
...
function createApp() {
  ...
  app.post('/api/ecommerce/init', (req, res) => {
    try {
  ...
  app.post('/api/ecommerce/process', (req, res) => {
    try {
  ...
  app.get('/api/ecommerce/audit', (req, res) => {
    res.status(200).json(service.getAuditTrail());
  });

  // Route sécurité/role
  app.post('/api/ecommerce/secure', (req, res) => {
...
describe('API ServiceEcommerce (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/ecommerce/init initialise le service', async () => {
    const res = await request(app)
      .post('/api/ecommerce/init')
      .send({ config: { mode: 'ultra', secure: true } });
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
  });

  test('POST /api/ecommerce/process exécute une opération', async () => {
    await request(app).post('/api/ecommerce/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/ecommerce/process')
  ...
  test('POST /api/ecommerce/process refuse une opération invalide', async () => {
    await request(app).post('/api/ecommerce/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/ecommerce/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/ecommerce/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/ecommerce/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/ecommerce/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/ecommerce/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/ecommerce/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/ecommerce/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/ecommerce/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/ecommerce/secure')
