"""
service_ecommerce.py - Service principal ecommerce (Python)
...
def get_ecommerce_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleecommerce_{model_id}", "data": "..."}


def list_ecommerce_models():
    return [get_ecommerce_model(i) for i in range(1, 4)]
