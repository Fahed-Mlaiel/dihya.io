"""
service_tourisme.py - Service principal tourisme (Python)
...
def get_tourisme_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèletourisme_{model_id}", "data": "..."}


def list_tourisme_models():
    return [get_tourisme_model(i) for i in range(1, 4)]
