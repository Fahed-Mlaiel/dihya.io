/**
 * @file test_service_tourisme.js
 * @description Tests ultra avancés clé en main pour ServiceTourisme (JS)
...
/**
 * Initialisation avancée du service métier tourisme pour chaque test.
 */
describe('ServiceTourisme (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceTourisme({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (gettourismeModel, listtourismeModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_tourisme');

  test('gettourismeModel retourne un modèle valide', () => {
    const model = serviceModule.gettourismeModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('gettourismeModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.gettourismeModel()).toThrow('id requis');
  });

  test('listtourismeModels retourne une liste de modèles', () => {
    const models = serviceModule.listtourismeModels();
