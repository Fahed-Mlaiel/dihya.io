"""
service_hotellerie.py - Service principal hotellerie (Python)
...
def get_hotellerie_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlehotellerie_{model_id}", "data": "..."}


def list_hotellerie_models():
    return [get_hotellerie_model(i) for i in range(1, 4)]
