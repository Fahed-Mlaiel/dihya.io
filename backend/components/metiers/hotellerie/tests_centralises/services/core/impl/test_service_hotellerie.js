/**
 * @file test_service_hotellerie.js
 * @description Tests ultra avancés clé en main pour ServiceHotellerie (JS)
...
/**
 * Initialisation avancée du service métier hotellerie pour chaque test.
 */
describe('ServiceHotellerie (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceHotellerie({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (gethotellerieModel, listhotellerieModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_hotellerie');

  test('gethotellerieModel retourne un modèle valide', () => {
    const model = serviceModule.gethotellerieModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('gethotellerieModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.gethotellerieModel()).toThrow('id requis');
  });

  test('listhotellerieModels retourne une liste de modèles', () => {
    const models = serviceModule.listhotellerieModels();
