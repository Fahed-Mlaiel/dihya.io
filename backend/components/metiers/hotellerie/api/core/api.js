// api.js – Point d’entrée Express ultra avancé pour l’API Hotellerie
const express = require('express');
const router = express.Router();
const HotellerieController = require('../core/controllers/hotellerie_controller');
...
// GET hotellerie entity by ID
router.get('/hotellerie/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await HotellerieController.getById(req.params.id);
...
// POST create hotellerie entity
router.post('/hotellerie', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await HotellerieController.create(req.body);
...
// PUT update hotellerie entity
router.put('/hotellerie/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await HotellerieController.update(req.params.id, req.body);
...
// DELETE hotellerie entity
router.delete('/hotellerie/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await HotellerieController.delete(req.params.id);
