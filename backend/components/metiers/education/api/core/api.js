// api.js – Point d’entrée Express ultra avancé pour l’API Education
const express = require('express');
const router = express.Router();
const EducationController = require('../core/controllers/education_controller');
...
// GET education entity by ID
router.get('/education/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await EducationController.getById(req.params.id);
...
// POST create education entity
router.post('/education', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await EducationController.create(req.body);
...
// PUT update education entity
router.put('/education/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await EducationController.update(req.params.id, req.body);
...
router.delete('/education/:id', async (req, res) => {
  ...
  try {
    await EducationController.delete(req.params.id);
