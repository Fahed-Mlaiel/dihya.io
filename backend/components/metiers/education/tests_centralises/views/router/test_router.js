...
describe('Views Router Central Tests', () => {
    ...
    it('GET /education retourne une liste vide', async () => {
        const res = await request(app).get('/education');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /education crée un objet education avec valeurs par défaut', async () => {
        const res = await request(app).post('/education').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /education crée un objet education avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/education').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /education RGPD: ne retourne pas de données sensibles', async () => {
