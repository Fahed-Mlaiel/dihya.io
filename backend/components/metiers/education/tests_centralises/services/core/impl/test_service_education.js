/**
 * @file test_service_education.js
 * @description Tests ultra avancés clé en main pour ServiceEducation (JS)
...
/**
 * Initialisation avancée du service métier education pour chaque test.
 */
describe('ServiceEducation (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceEducation({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (geteducationModel, listeducationModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_education');

  test('geteducationModel retourne un modèle valide', () => {
    const model = serviceModule.geteducationModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('geteducationModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.geteducationModel()).toThrow('id requis');
  });

  test('listeducationModels retourne une liste de modèles', () => {
    const models = serviceModule.listeducationModels();
