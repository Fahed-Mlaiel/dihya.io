...
const { rendereducationTemplateUltra, validateeducationTemplateUltra } = require('./template_education');

describe('Templates Core Central Tests', () => {
    it('should render a valid education template (nominal)', () => {
        const data = { id: '1', name: 'Cube', meta: { type: 'mesh' }, format: 'obj', i18n: { fr: 'Cube' } };
        const options = { audit: 'ok', accessibility: 'AA', rgpd: 'ok' };
        const result = rendereducationTemplateUltra(data, options);
    ...
    it('should throw on missing id or name', () => {
        expect(() => rendereducationTemplateUltra({ name: 'Cube' })).toThrow();
        expect(() => rendereducationTemplateUltra({ id: '1' })).toThrow();
        expect(() => rendereducationTemplateUltra({})).toThrow();
    });
    ...
    it('should validate education template (nominal, RGPD ok)', () => {
        const data = { id: '3', name: 'Pyramid', rgpd: 'ok' };
        expect(validateeducationTemplateUltra(data)).toBe(true);
    });

    it('should invalidate education template (missing id/name or RGPD)', () => {
        expect(validateeducationTemplateUltra({ name: 'NoId' })).toBe(false);
        expect(validateeducationTemplateUltra({ id: '4' })).toBe(false);
        expect(validateeducationTemplateUltra({ id: '5', name: 'Bad', rgpd: 'ko' })).toBe(false);
        expect(validateeducationTemplateUltra(null)).toBe(false);
    });
