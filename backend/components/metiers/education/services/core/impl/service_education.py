"""
service_education.py - Service principal education (Python)
...
def get_education_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleeducation_{model_id}", "data": "..."}


def list_education_models():
    return [get_education_model(i) for i in range(1, 4)]
