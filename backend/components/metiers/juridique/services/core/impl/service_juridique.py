"""
service_juridique.py - Service principal juridique (Python)
...
def get_juridique_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlejuridique_{model_id}", "data": "..."}


def list_juridique_models():
    return [get_juridique_model(i) for i in range(1, 4)]
