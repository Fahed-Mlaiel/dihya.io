const { auditJuridique } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditJuridique({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditJuridique({ status: 'inactive' });
