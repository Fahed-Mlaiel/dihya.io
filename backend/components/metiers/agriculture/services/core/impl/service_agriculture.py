"""
service_agriculture.py - Service principal agriculture (Python)
...
def get_agriculture_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleagriculture_{model_id}", "data": "..."}


def list_agriculture_models():
    return [get_agriculture_model(i) for i in range(1, 4)]
