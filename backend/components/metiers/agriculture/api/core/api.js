// api.js – Point d’entrée Express ultra avancé pour l’API Agriculture
const express = require('express');
const router = express.Router();
const AgricultureController = require('../core/controllers/agriculture_controller');
...
// GET agriculture entity by ID
router.get('/agriculture/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await AgricultureController.getById(req.params.id);
...
// POST create agriculture entity
router.post('/agriculture', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await AgricultureController.create(req.body);
...
// PUT update agriculture entity
router.put('/agriculture/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await AgricultureController.update(req.params.id, req.body);
...
// DELETE agriculture entity
router.delete('/agriculture/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await AgricultureController.delete(req.params.id);
