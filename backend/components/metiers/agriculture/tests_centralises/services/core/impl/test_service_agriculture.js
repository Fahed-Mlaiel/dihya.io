/**
 * @file test_service_agriculture.js
 * @description Tests ultra avancés clé en main pour ServiceAgriculture (JS)
...
/**
 * Initialisation avancée du service métier agriculture pour chaque test.
 */
describe('ServiceAgriculture (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceAgriculture({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getagricultureModel, listagricultureModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_agriculture');

  test('getagricultureModel retourne un modèle valide', () => {
    const model = serviceModule.getagricultureModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getagricultureModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getagricultureModel()).toThrow('id requis');
  });

  test('listagricultureModels retourne une liste de modèles', () => {
    const models = serviceModule.listagricultureModels();
