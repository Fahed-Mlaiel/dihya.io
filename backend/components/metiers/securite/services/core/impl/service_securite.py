"""
service_securite.py - Service principal securite (Python)
...
def get_securite_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlesecurite_{model_id}", "data": "..."}


def list_securite_models():
    return [get_securite_model(i) for i in range(1, 4)]
