// api.js – Point d’entrée Express ultra avancé pour l’API Social
const express = require('express');
const router = express.Router();
const SocialController = require('../core/controllers/social_controller');
...
// GET social entity by ID
router.get('/social/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await SocialController.getById(req.params.id);
...
// POST create social entity
router.post('/social', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await SocialController.create(req.body);
...
// PUT update social entity
router.put('/social/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await SocialController.update(req.params.id, req.body);
...
router.delete('/social/:id', async (req, res) => {
  ...
  try {
    await SocialController.delete(req.params.id);
