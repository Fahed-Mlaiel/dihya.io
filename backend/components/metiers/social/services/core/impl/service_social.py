"""
service_social.py - Service principal social (Python)
...
def get_social_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlesocial_{model_id}", "data": "..."}


def list_social_models():
    return [get_social_model(i) for i in range(1, 4)]
