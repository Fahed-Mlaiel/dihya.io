/**
 * @file test_social_controller.js
 * @description Tests ultra avancés clé en main pour le contrôleur API social (JS)
...
// Mock contrôleur API social ultra avancé pour tests
function createControllerApp() {
  ...
  app.post('/api/social/import', (req, res) => {
  ...
  app.get('/api/social/export', (req, res) => {
    // Simulation d'export avancé
    res.status(200).json({ success: true, data: { id: 1, name: 'Exportsocial' } });
  });

  // Route de gestion des rôles et sécurité
  app.post('/api/social/role', (req, res) => {
  ...
  app.post('/api/social/fonction', (req, res) => {
    try {
...
describe('social Controller API (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createControllerApp();
  });

  test('POST /api/social/import effectue un import avancé', async () => {
  ...
  test('GET /api/social/export effectue un export avancé', async () => {
    const res = await request(app).get('/api/social/export');
    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.data).toHaveProperty('id');
  });

  test('POST /api/social/role refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/social/role')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/social/role accepte un admin', async () => {
    const res = await request(app)
      .post('/api/social/role')
  ...
  test('POST /api/social/fonction exécute une fonction métier avancée', async () => {
    const res = await request(app)
      .post('/api/social/fonction')
