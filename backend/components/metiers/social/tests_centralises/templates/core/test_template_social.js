...
const { rendersocialTemplateUltra, validatesocialTemplateUltra } = require('./template_social');

describe('Templates Core Central Tests', () => {
    it('should render a valid social template (nominal)', () => {
        const data = { id: '1', name: 'Cube', meta: { type: 'mesh' }, format: 'obj', i18n: { fr: 'Cube' } };
        const options = { audit: 'ok', accessibility: 'AA', rgpd: 'ok' };
        const result = rendersocialTemplateUltra(data, options);
    ...
    it('should throw on missing id or name', () => {
        expect(() => rendersocialTemplateUltra({ name: 'Cube' })).toThrow();
        expect(() => rendersocialTemplateUltra({ id: '1' })).toThrow();
        expect(() => rendersocialTemplateUltra({})).toThrow();
    });
    ...
    it('should validate social template (nominal, RGPD ok)', () => {
        const data = { id: '3', name: 'Pyramid', rgpd: 'ok' };
        expect(validatesocialTemplateUltra(data)).toBe(true);
    });

    it('should invalidate social template (missing id/name or RGPD)', () => {
        expect(validatesocialTemplateUltra({ name: 'NoId' })).toBe(false);
        expect(validatesocialTemplateUltra({ id: '4' })).toBe(false);
        expect(validatesocialTemplateUltra({ id: '5', name: 'Bad', rgpd: 'ko' })).toBe(false);
        expect(validatesocialTemplateUltra(null)).toBe(false);
    });
