...
describe('Views Router Central Tests', () => {
    ...
    it('GET /social retourne une liste vide', async () => {
        const res = await request(app).get('/social');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /social crée un objet social avec valeurs par défaut', async () => {
        const res = await request(app).post('/social').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /social crée un objet social avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/social').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /social RGPD: ne retourne pas de données sensibles', async () => {
