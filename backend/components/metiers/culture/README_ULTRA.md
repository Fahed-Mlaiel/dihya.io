# Culture (culture) – Documentation Ultra Avancée
...
- Cela permet d’importer tous les modules métier (`backend.components.metiers.culture`, etc.) sans erreur.
- Cette commande est compatible Linux/Bash (par défaut dans ce projet).
- Pour les tests JavaScript, utilisez :

```bash
npm test --prefix backend/components/metiers/culture
```
...
- Les imports doivent être absolus pour tous les modules inter-métiers (ex : `from backend.components.metiers.culture...`).
