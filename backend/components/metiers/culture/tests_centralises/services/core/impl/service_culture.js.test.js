/**
 * @file test_service_voyage.js
 * @description Tests ultra avancés clé en main pour ServiceVoyage (JS)
...
/**
 * Initialisation avancée du service métier voyage pour chaque test.
 */
describe('ServiceVoyage (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceVoyage({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getvoyageModel, listvoyageModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_voyage');

  test('getvoyageModel retourne un modèle valide', () => {
    const model = serviceModule.getvoyageModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getvoyageModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getvoyageModel()).toThrow('id requis');
  });

  test('listvoyageModels retourne une liste de modèles', () => {
    const models = serviceModule.listvoyageModels();
