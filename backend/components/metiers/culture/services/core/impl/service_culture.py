"""
service_culture.py - Service principal culture (Python)
...
def get_culture_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleculture_{model_id}", "data": "..."}


def list_culture_models():
    return [get_culture_model(i) for i in range(1, 4)]
