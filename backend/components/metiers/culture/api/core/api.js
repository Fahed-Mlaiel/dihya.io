// api.js – Point d’entrée Express ultra avancé pour l’API Culture
const express = require('express');
const router = express.Router();
const CultureController = require('../core/controllers/culture_controller');
...
// GET culture entity by ID
router.get('/culture/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await CultureController.getById(req.params.id);
...
// POST create culture entity
router.post('/culture', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await CultureController.create(req.body);
...
// PUT update culture entity
router.put('/culture/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await CultureController.update(req.params.id, req.body);
