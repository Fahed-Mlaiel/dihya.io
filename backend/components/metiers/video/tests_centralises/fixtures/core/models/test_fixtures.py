...
import pytest
from backend.components.metiers.video.fixtures.core.models.fixtures import (
    sample_video_asset,
    advanced_video_model,
    asset_texture,
    user_admin,
)
...
def test_sample_asset(sample_video_asset):
    asset = sample_video_asset
    assert asset["name"] == "Cube"

def test_advanced_model(advanced_video_model):
