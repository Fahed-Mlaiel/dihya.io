...
from video.fixtures.core.validators.fixtures_validator import (
...
def test___init__():
    assert callable(is_valid_video_model)
    assert callable(is_valid_user)


def test___init___basic():
    assert is_valid_video_model({"id": "abc", "vertices": [], "faces": 0}) is True
