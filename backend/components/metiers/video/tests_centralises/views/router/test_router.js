...
describe('Views Router Central Tests', () => {
    ...
    it('GET /video retourne une liste vide', async () => {
        const res = await request(app).get('/video');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /video crée un objet video avec valeurs par défaut', async () => {
        const res = await request(app).post('/video').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /video crée un objet video avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/video').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /video RGPD: ne retourne pas de données sensibles', async () => {
