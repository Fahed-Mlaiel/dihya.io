/**
 * @file test_service_video.js
 * @description Tests ultra avancés clé en main pour ServiceVideo (JS)
...
/**
 * Initialisation avancée du service métier video pour chaque test.
 */
describe('ServiceVideo (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceVideo({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getvideoModel, listvideoModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_video');

  test('getvideoModel retourne un modèle valide', () => {
    const model = serviceModule.getvideoModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getvideoModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getvideoModel()).toThrow('id requis');
  });

  test('listvideoModels retourne une liste de modèles', () => {
    const models = serviceModule.listvideoModels();
