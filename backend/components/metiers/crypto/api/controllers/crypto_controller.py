"""
crypto_controller.py – Contrôleur ultra avancé API Crypto (Python)
...
from ..validators.validators import validate_crypto_entity
...
def get_crypto(id: int):
    before_action("read", {"id": id})
    entity = db_find_by_id("crypto", id)
    if not entity:
        return None
...
@router.post("/crypto")
def create_crypto(data: dict):
    before_action("create", data)
    validate_crypto_entity(data)
    created = db_insert("crypto", data)
...
def update_crypto(id: int, data: dict):
    before_action("update", {"id": id, **data})
    validate_crypto_entity(data)
    updated = db_update("crypto", id, data)
...
def delete_crypto(id: int):
    ...
    deleted = db_delete("crypto", id)
    audit_entity({"id": id}, "delete")
    after_action("delete", {"id": id})
    return deleted


# Es gibt keine Klasse CryptoController, sondern nur Funktions-Exporte.
...
class CryptoController:
    @staticmethod
    def get_by_id(id):
        return get_crypto(id)

    @staticmethod
    def create(data):
        return create_crypto(data)

    @staticmethod
    def update(id, data):
        return update_crypto(id, data)

    @staticmethod
    def delete(id):
        return delete_crypto(id)
