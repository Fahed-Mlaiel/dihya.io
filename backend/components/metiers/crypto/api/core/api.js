// api.js – Point d’entrée Express ultra avancé pour l’API Crypto
const express = require('express');
const router = express.Router();
const CryptoController = require('../core/controllers/crypto_controller');
...
// GET crypto entity by ID
router.get('/crypto/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await CryptoController.getById(req.params.id);
...
// POST create crypto entity
router.post('/crypto', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await CryptoController.create(req.body);
...
// PUT update crypto entity
router.put('/crypto/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await CryptoController.update(req.params.id, req.body);
