"""
service_crypto.py - Service principal crypto (Python)
...
def get_crypto_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlecrypto_{model_id}", "data": "..."}


def list_crypto_models():
    return [get_crypto_model(i) for i in range(1, 4)]
