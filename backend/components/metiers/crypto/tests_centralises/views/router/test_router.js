...
describe('Views Router Central Tests', () => {
    ...
    it('GET /crypto retourne une liste vide', async () => {
        const res = await request(app).get('/crypto');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /crypto crée un objet crypto avec valeurs par défaut', async () => {
        const res = await request(app).post('/crypto').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /crypto crée un objet crypto avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/crypto').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /crypto RGPD: ne retourne pas de données sensibles', async () => {
