// api.js – Point d’entrée Express ultra avancé pour l’API Blockchain
const express = require('express');
const router = express.Router();
const BlockchainController = require('../core/controllers/blockchain_controller');
...
// GET blockchain entity by ID
router.get('/blockchain/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await BlockchainController.getById(req.params.id);
...
// POST create blockchain entity
router.post('/blockchain', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await BlockchainController.create(req.body);
...
// PUT update blockchain entity
router.put('/blockchain/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await BlockchainController.update(req.params.id, req.body);
...
// DELETE blockchain entity
router.delete('/blockchain/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await BlockchainController.delete(req.params.id);
