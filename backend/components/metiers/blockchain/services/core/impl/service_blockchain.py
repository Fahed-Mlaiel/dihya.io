"""
service_blockchain.py - Service principal blockchain (Python)
...
def get_blockchain_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleblockchain_{model_id}", "data": "..."}


def list_blockchain_models():
    return [get_blockchain_model(i) for i in range(1, 4)]
