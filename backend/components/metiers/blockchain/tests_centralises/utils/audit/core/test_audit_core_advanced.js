const { auditBlockchain } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditBlockchain({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditBlockchain({ status: 'inactive' });
