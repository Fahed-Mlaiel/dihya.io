/**
 * @file test_service_blockchain.js
 * @description Tests ultra avancés clé en main pour ServiceBlockchain (JS)
...
/**
 * Initialisation avancée du service métier blockchain pour chaque test.
 */
describe('ServiceBlockchain (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceBlockchain({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getblockchainModel, listblockchainModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_blockchain');

  test('getblockchainModel retourne un modèle valide', () => {
    const model = serviceModule.getblockchainModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getblockchainModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getblockchainModel()).toThrow('id requis');
  });

  test('listblockchainModels retourne une liste de modèles', () => {
    const models = serviceModule.listblockchainModels();
