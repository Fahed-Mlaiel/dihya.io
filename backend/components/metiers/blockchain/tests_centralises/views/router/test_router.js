...
describe('Views Router Central Tests', () => {
    ...
    it('GET /blockchain retourne une liste vide', async () => {
        const res = await request(app).get('/blockchain');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /blockchain crée un objet blockchain avec valeurs par défaut', async () => {
        const res = await request(app).post('/blockchain').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /blockchain crée un objet blockchain avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/blockchain').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /blockchain RGPD: ne retourne pas de données sensibles', async () => {
