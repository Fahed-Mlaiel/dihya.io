...
import pytest
from backend.components.metiers.mobile.utils.validators.core import validators

def test_validate_mobile_nominal():
    data = {"nom": "Projet X", "statut": "actif"}
    assert validators.validate_mobile(data) is True

def test_validate_mobile_missing_nom():
    data = {"statut": "actif"}
    with pytest.raises(ValueError, match="Nom requis"):
        validators.validate_mobile(data)

def test_validate_mobile_invalid_statut():
    data = {"nom": "Projet Y", "statut": "supprimé"}
    with pytest.raises(ValueError, match="Statut invalide"):
        validators.validate_mobile(data)

def test_validate_mobile_statut_none():
    data = {"nom": "Projet Z", "statut": None}
    assert validators.validate_mobile(data) is True

def test_validate_mobile_statut_archived():
    data = {"nom": "Projet A", "statut": "archivé"}
    assert validators.validate_mobile(data) is True

def test_validate_mobile_extra_fields():
