/**
 * @file test_service_mobile.js
 * @description Tests ultra avancés clé en main pour ServiceMobile (JS)
...
/**
 * Initialisation avancée du service métier mobile pour chaque test.
 */
describe('ServiceMobile (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceMobile({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getmobileModel, listmobileModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_mobile');

  test('getmobileModel retourne un modèle valide', () => {
    const model = serviceModule.getmobileModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getmobileModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getmobileModel()).toThrow('id requis');
  });

  test('listmobileModels retourne une liste de modèles', () => {
    const models = serviceModule.listmobileModels();
