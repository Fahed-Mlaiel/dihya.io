 ...
 * @description Tests ultra avancés clé en main pour l'API ServiceMobile (JS)
...
function createApp() {
  ...
  app.post('/api/mobile/init', (req, res) => {
    try {
  ...
  app.post('/api/mobile/process', (req, res) => {
    try {
  ...
  // Route sécurité/role
  app.post('/api/mobile/secure', (req, res) => {
...
describe('API ServiceMobile (Ultra Avancé)', () => {
  let app;
  beforeEach(() => {
    app = createApp();
  });

  test('POST /api/mobile/init initialise le service', async () => {
  ...
  test('POST /api/mobile/process exécute une opération', async () => {
    await request(app).post('/api/mobile/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
  ...
  test('POST /api/mobile/process refuse une opération invalide', async () => {
    await request(app).post('/api/mobile/init').send({ config: { mode: 'ultra' } });
    const res = await request(app)
      .post('/api/mobile/process')
      .send({ operation: '', data: {} });
  ...
  test('GET /api/mobile/audit retourne l\'audit trail', async () => {
    await request(app).post('/api/mobile/init').send({ config: { mode: 'ultra' } });
    await request(app).post('/api/mobile/process').send({ operation: 'generate', data: {} });
    const res = await request(app).get('/api/mobile/audit');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
  });

  test('POST /api/mobile/secure refuse un rôle non admin', async () => {
    const res = await request(app)
      .post('/api/mobile/secure')
      .send({ user: { id: 2, role: 'user' }, action: 'delete' });
    expect(res.statusCode).toBe(403);
    expect(res.body.error).toMatch(/Accès refusé/);
  });

  test('POST /api/mobile/secure accepte un admin', async () => {
    const res = await request(app)
      .post('/api/mobile/secure')
