// api.js – Point d’entrée Express ultra avancé pour l’API Mobile
const express = require('express');
const router = express.Router();
const MobileController = require('../core/controllers/mobile_controller');
...
// GET mobile entity by ID
router.get('/mobile/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await MobileController.getById(req.params.id);
...
// POST create mobile entity
router.post('/mobile', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await MobileController.create(req.body);
...
// PUT update mobile entity
router.put('/mobile/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await MobileController.update(req.params.id, req.body);
