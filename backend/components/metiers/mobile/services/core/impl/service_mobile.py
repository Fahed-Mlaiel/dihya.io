"""
service_mobile.py - Service principal mobile (Python)
...
def get_mobile_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlemobile_{model_id}", "data": "..."}


def list_mobile_models():
    return [get_mobile_model(i) for i in range(1, 4)]
