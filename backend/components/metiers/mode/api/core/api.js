// api.js – Point d’entrée Express ultra avancé pour l’API Mode
const express = require('express');
const router = express.Router();
const ModeController = require('../core/controllers/mode_controller');
...
// GET mode entity by ID
router.get('/mode/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await ModeController.getById(req.params.id);
...
// POST create mode entity
router.post('/mode', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await ModeController.create(req.body);
...
// PUT update mode entity
router.put('/mode/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await ModeController.update(req.params.id, req.body);
...
router.delete('/mode/:id', async (req, res) => {
  ...
  try {
    await ModeController.delete(req.params.id);
