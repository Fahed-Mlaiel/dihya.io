"""
service_mode.py - Service principal mode (Python)
...
def get_mode_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlemode_{model_id}", "data": "..."}


def list_mode_models():
    return [get_mode_model(i) for i in range(1, 4)]
