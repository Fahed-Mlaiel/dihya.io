/**
 * @file test_service_mode.js
 * @description Tests ultra avancés clé en main pour ServiceMode (JS)
...
/**
 * Initialisation avancée du service métier mode pour chaque test.
 */
describe('ServiceMode (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceMode({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getmodeModel, listmodeModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_mode');

  test('getmodeModel retourne un modèle valide', () => {
    const model = serviceModule.getmodeModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getmodeModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getmodeModel()).toThrow('id requis');
  });

  test('listmodeModels retourne une liste de modèles', () => {
    const models = serviceModule.listmodeModels();
