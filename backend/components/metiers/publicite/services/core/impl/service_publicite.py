"""
service_publicite.py - Service principal publicite (Python)
...
def get_publicite_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlepublicite_{model_id}", "data": "..."}


def list_publicite_models():
    return [get_publicite_model(i) for i in range(1, 4)]
