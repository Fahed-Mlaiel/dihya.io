// api.js – Point d’entrée Express ultra avancé pour l’API Banque_Finance
const express = require('express');
const router = express.Router();
const Banque_FinanceController = require('../core/controllers/banque_finance_controller');
...
// GET banque_finance entity by ID
router.get('/banque_finance/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await Banque_FinanceController.getById(req.params.id);
...
// POST create banque_finance entity
router.post('/banque_finance', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await Banque_FinanceController.create(req.body);
...
// PUT update banque_finance entity
router.put('/banque_finance/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await Banque_FinanceController.update(req.params.id, req.body);
...
// DELETE banque_finance entity
router.delete('/banque_finance/:id', async (req, res) => {
  beforeAction('delete', { id: req.params.id });
  try {
    await Banque_FinanceController.delete(req.params.id);
