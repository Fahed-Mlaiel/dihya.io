...
def test_guide_fonctionnel_exemple_output(capsys):
    from backend.components.metiers.banque_finance.legacy.documentation.guides.fonctionnelle import (
        guide_fonctionnel_exemple,
    )

    guide_fonctionnel_exemple.guide_fonctionnel_exemple()
    captured = capsys.readouterr()
    assert "Guide fonctionnel legacy Banque_Finance" in captured.out
