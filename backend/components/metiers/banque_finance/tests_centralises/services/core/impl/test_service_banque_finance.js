/**
 * @file test_service_banque_finance.js
 * @description Tests ultra avancés clé en main pour ServiceBanque_Finance (JS)
...
/**
 * Initialisation avancée du service métier banque_finance pour chaque test.
 */
describe('ServiceBanque_Finance (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceBanque_Finance({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getbanque_financeModel, listbanque_financeModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_banque_finance');

  test('getbanque_financeModel retourne un modèle valide', () => {
    const model = serviceModule.getbanque_financeModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getbanque_financeModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getbanque_financeModel()).toThrow('id requis');
  });

  test('listbanque_financeModels retourne une liste de modèles', () => {
    const models = serviceModule.listbanque_financeModels();
