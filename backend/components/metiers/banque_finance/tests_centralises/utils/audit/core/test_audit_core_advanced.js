const { auditBanque_Finance } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditBanque_Finance({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditBanque_Finance({ status: 'inactive' });
