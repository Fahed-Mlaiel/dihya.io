"""
service_banque_finance.py - Service principal banque_finance (Python)
...
def get_banque_finance_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlebanque_finance_{model_id}", "data": "..."}


def list_banque_finance_models():
    return [get_banque_finance_model(i) for i in range(1, 4)]
