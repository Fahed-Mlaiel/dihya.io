...
describe('Views Router Central Tests', () => {
    ...
    it('GET /voice retourne une liste vide', async () => {
        const res = await request(app).get('/voice');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /voice crée un objet voice avec valeurs par défaut', async () => {
        const res = await request(app).post('/voice').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /voice crée un objet voice avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/voice').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /voice RGPD: ne retourne pas de données sensibles', async () => {
