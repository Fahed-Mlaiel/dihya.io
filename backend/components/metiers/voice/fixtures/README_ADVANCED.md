# Fixtures voice – Documentation Ultra Avancée (Mise à jour 2025)
...
### Validation (JS)
```js
const { isValidvoiceModel } = require('./helpers/validators');
assert(isValidvoiceModel(model));
```

### Validation (Python)
```python
from .helpers.validators import is_valid_voice_model
assert is_valid_voice_model(model)
```
