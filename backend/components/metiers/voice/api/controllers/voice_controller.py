"""
voice_controller.py – Contrôleur ultra avancé API Voice (Python)
Inclut : logique métier, RGPD, audit, accessibilité, hooks, edge cases
"""

from fastapi import APIRouter
from ..db.db import db_find_by_id, db_insert, db_update, db_delete
from ..validators.validators import validate_voice_entity
from ..audit.audit import audit_entity
from ..rgpd.rgpd import rgpd_sanitize
from ..accessibility.accessibility import check_accessibility
from ..hooks.hooks import before_action, after_action

router = APIRouter()


@router.get("/voice/{id}")
def get_voice(id: int):
    before_action("read", {"id": id})
    entity = db_find_by_id("voice", id)
    if not entity:
        return None
    entity = rgpd_sanitize(entity)
    check_accessibility(entity)
    audit_entity(entity, "read")
    after_action("read", entity)
    return entity


@router.post("/voice")
def create_voice(data: dict):
    before_action("create", data)
    validate_voice_entity(data)
    created = db_insert("voice", data)
    audit_entity(created, "create")
    after_action("create", created)
    return rgpd_sanitize(created)


@router.put("/voice/{id}")
def update_voice(id: int, data: dict):
    before_action("update", {"id": id, **data})
    validate_voice_entity(data)
    updated = db_update("voice", id, data)
    audit_entity(updated, "update")
    after_action("update", updated)
    return rgpd_sanitize(updated)


@router.delete("/voice/{id}")
def delete_voice(id: int):
    before_action("delete", {"id": id})
    deleted = db_delete("voice", id)
    audit_entity({"id": id}, "delete")
    after_action("delete", {"id": id})
    return deleted


class voiceController:
    @staticmethod
    def get_by_id(id):
        return get_voice(id)

    @staticmethod
    def create(data):
        return create_voice(data)

    @staticmethod
    def update(id, data):
        return update_voice(id, data)

    @staticmethod
    def delete(id):
        return delete_voice(id)
