// api.js – Point d’entrée Express ultra avancé pour l’API voice
const express = require('express');
const router = express.Router();
const voiceController = require('../core/controllers/voice_controller');
...
// GET voice entity by ID
router.get('/voice/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await voiceController.getById(req.params.id);
...
// POST create voice entity
router.post('/voice', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await voiceController.create(req.body);
...
// PUT update voice entity
router.put('/voice/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await voiceController.update(req.params.id, req.body);
...
router.delete('/voice/:id', async (req, res) => {
  ...
  try {
    await voiceController.delete(req.params.id);
