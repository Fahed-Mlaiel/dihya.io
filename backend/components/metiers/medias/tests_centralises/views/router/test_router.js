...
describe('Views Router Central Tests', () => {
    ...
    it('GET /medias retourne une liste vide', async () => {
        const res = await request(app).get('/medias');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /medias crée un objet medias avec valeurs par défaut', async () => {
        const res = await request(app).post('/medias').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /medias crée un objet medias avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/medias').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /medias RGPD: ne retourne pas de données sensibles', async () => {
