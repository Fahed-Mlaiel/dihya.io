...
import pytest
from backend.components.metiers.medias.utils.validators.core import validators

def test_validate_medias_nominal():
    data = {"nom": "Projet X", "statut": "actif"}
    assert validators.validate_medias(data) is True

def test_validate_medias_missing_nom():
    data = {"statut": "actif"}
    with pytest.raises(ValueError, match="Nom requis"):
        validators.validate_medias(data)

def test_validate_medias_invalid_statut():
    data = {"nom": "Projet Y", "statut": "supprimé"}
    with pytest.raises(ValueError, match="Statut invalide"):
        validators.validate_medias(data)

def test_validate_medias_statut_none():
    data = {"nom": "Projet Z", "statut": None}
    assert validators.validate_medias(data) is True

def test_validate_medias_statut_archived():
    data = {"nom": "Projet A", "statut": "archivé"}
    assert validators.validate_medias(data) is True

def test_validate_medias_extra_fields():
