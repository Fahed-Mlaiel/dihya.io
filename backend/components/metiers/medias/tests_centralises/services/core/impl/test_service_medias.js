/**
 * @file test_service_medias.js
 * @description Tests ultra avancés clé en main pour ServiceMedias (JS)
...
/**
 * Initialisation avancée du service métier medias pour chaque test.
 */
describe('ServiceMedias (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceMedias({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getmediasModel, listmediasModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_medias');

  test('getmediasModel retourne un modèle valide', () => {
    const model = serviceModule.getmediasModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getmediasModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getmediasModel()).toThrow('id requis');
  });

  test('listmediasModels retourne une liste de modèles', () => {
    const models = serviceModule.listmediasModels();
