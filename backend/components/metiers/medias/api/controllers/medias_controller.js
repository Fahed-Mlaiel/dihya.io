// medias_controller.js – Contrôleur ultra avancé API Medias (JS)
const db = require('../db/db');
const { validatemediasEntity } = require('../validators/validators');
...
const MediasController = {
  async getById(id) {
    ...
    let entity = db.findById('medias', id);
  ...
  async create(data) {
    ...
    validatemediasEntity(data);
    const created = db.insert('medias', data);
  ...
  async update(id, data) {
    ...
    validatemediasEntity(data);
    const updated = db.update('medias', id, data);
  ...
  async delete(id) {
    ...
    const deleted = db.delete('medias', id);
