// api.js – Point d’entrée Express ultra avancé pour l’API Medias
const express = require('express');
const router = express.Router();
const MediasController = require('../core/controllers/medias_controller');
...
// GET medias entity by ID
router.get('/medias/:id', async (req, res) => {
  beforeAction('read', { id: req.params.id });
  try {
    let entity = await MediasController.getById(req.params.id);
...
// POST create medias entity
router.post('/medias', async (req, res) => {
  beforeAction('create', req.body);
  try {
    const created = await MediasController.create(req.body);
...
// PUT update medias entity
router.put('/medias/:id', async (req, res) => {
  beforeAction('update', { id: req.params.id, ...req.body });
  try {
    const updated = await MediasController.update(req.params.id, req.body);
