"""
service_medias.py - Service principal medias (Python)
...
def get_medias_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlemedias_{model_id}", "data": "..."}


def list_medias_models():
    return [get_medias_model(i) for i in range(1, 4)]
