/**
 * @file test_service_science.js
 * @description Tests ultra avancés clé en main pour ServiceScience (JS)
...
/**
 * Initialisation avancée du service métier science pour chaque test.
 */
describe('ServiceScience (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceScience({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getscienceModel, listscienceModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_science');

  test('getscienceModel retourne un modèle valide', () => {
    const model = serviceModule.getscienceModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getscienceModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getscienceModel()).toThrow('id requis');
  });

  test('listscienceModels retourne une liste de modèles', () => {
    const models = serviceModule.listscienceModels();
