...
import pytest
from backend.components.metiers.science.utils.validators.core import validators

def test_validate_science_nominal():
    data = {"nom": "Projet X", "statut": "actif"}
    assert validators.validate_science(data) is True

def test_validate_science_missing_nom():
    data = {"statut": "actif"}
    with pytest.raises(ValueError, match="Nom requis"):
        validators.validate_science(data)

def test_validate_science_invalid_statut():
    data = {"nom": "Projet Y", "statut": "supprimé"}
    with pytest.raises(ValueError, match="Statut invalide"):
        validators.validate_science(data)

def test_validate_science_statut_none():
    data = {"nom": "Projet Z", "statut": None}
    assert validators.validate_science(data) is True

def test_validate_science_statut_archived():
    data = {"nom": "Projet A", "statut": "archivé"}
    assert validators.validate_science(data) is True

def test_validate_science_extra_fields():
