"""
service_science.py - Service principal science (Python)
...
def get_science_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèlescience_{model_id}", "data": "..."}


def list_science_models():
    return [get_science_model(i) for i in range(1, 4)]
