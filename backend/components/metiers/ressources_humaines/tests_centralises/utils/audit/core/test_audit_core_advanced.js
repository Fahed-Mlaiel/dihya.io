const { auditRessources_humaines } = require('../../../../utils/audit/core/audit');
...
describe('Audit Core Utils', () => {
  ...
  it('retourne un score élevé pour status=active', () => {
    const res = auditRessources_humaines({ status: 'active' });
  ...
  it('retourne un score faible pour status inactif', () => {
    const res = auditRessources_humaines({ status: 'inactive' });
  ...
  it('gère les cas limites (status manquant)', () => {
    const res = auditRessources_humaines({});
