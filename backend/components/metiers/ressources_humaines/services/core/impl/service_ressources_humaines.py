"""
service_ressources_humaines.py - Service principal ressources_humaines (Python)
...
class ServiceRessources_humaines:
...
def get_ressources_humaines_model(model_id):
    if not model_id:
        raise ValueError("id requis")
    return {"id": model_id, "name": f"Modèleressources_humaines_{model_id}", "data": "..."}


def list_ressources_humaines_models():
    return [get_ressources_humaines_model(i) for i in range(1, 4)]
