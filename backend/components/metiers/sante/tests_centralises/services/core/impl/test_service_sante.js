/**
 * @file test_service_sante.js
 * @description Tests ultra avancés clé en main pour ServiceSante (JS)
...
/**
 * Initialisation avancée du service métier sante pour chaque test.
 */
describe('ServiceSante (Ultra Avancé)', () => {
  let service;
  beforeEach(() => {
    service = new ServiceSante({ audit: true });
  });
 ...
 */
describe('Fonctions API legacy (getsanteModel, listsanteModels, auditModel, secureAccess)', () => {
  const serviceModule = require('../../../../services/core/impl/service_sante');

  test('getsanteModel retourne un modèle valide', () => {
    const model = serviceModule.getsanteModel(5);
    expect(model).toEqual({ id: 5, name: 'Model 5' });
  });

  test('getsanteModel lève une erreur si id manquant', () => {
    expect(() => serviceModule.getsanteModel()).toThrow('id requis');
  });

  test('listsanteModels retourne une liste de modèles', () => {
    const models = serviceModule.listsanteModels();
