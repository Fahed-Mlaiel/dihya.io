...
describe('Views Router Central Tests', () => {
    ...
    it('GET /sante retourne une liste vide', async () => {
        const res = await request(app).get('/sante');
        expect(res.status).toBe(200);
        expect(res.body).toEqual({ d3s: [], total: 0 });
    });

    it('POST /sante crée un objet sante avec valeurs par défaut', async () => {
        const res = await request(app).post('/sante').send({});
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ nom: '', description: '', type: 'objet' });
    });

    it('POST /sante crée un objet sante avec données fournies', async () => {
        const data = { nom: 'Cube', description: 'Test', type: 'mesh' };
        const res = await request(app).post('/sante').send(data);
        expect(res.status).toBe(201);
        expect(res.body).toEqual(data);
    });

    it('POST /sante RGPD: ne retourne pas de données sensibles', async () => {
