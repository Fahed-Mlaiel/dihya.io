/**
 * @file test_mode.js
 * @description Tests d'intégration avancés pour l'API mode (REST/GraphQL, sécurité, i18n, plugins, RGPD, etc.)
 * @author Dihya
 * @date 2025-05-25
 */
...
describe('Mode API - Sécurité, i18n, RGPD, Plugins, SEO', () => {
  it('GET /api/mode retourne la liste (auth, i18n)', async () => {
  ...
  it('POST /api/mode (admin)', async () => {
    const res = await request(app)
      .post('/api/mode')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ nom: 'Défilé IA', description: 'Mode automatisée' })
      .expect(201);
