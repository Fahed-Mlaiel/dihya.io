...
Ce dossier contient les tests d'intégration pour les routes et services sport (gestion événements, IA, plugins, RGPD, multilingue, logs, accessibilité, etc.) dans Dihya.

## Objectifs
- Vérifier l'intégration complète des APIs sport (événements, IA, logs, plugins, RGPD, SEO, accessibilité, etc.).
...
## Structure des tests
- **Unitaires** : Mock des services sport, validation schémas, sécurité, i18n.
- **Intégration** : Endpoints REST/GraphQL, gestion événements, logs, SEO, accessibilité.
- **E2E** : Scénarios réels multi-utilisateurs, multi-langues, multi-tenant, plugins dynamiques.

## Exécution
```bash
npm run test:integration sport
```
...
## Exemples de tests
- Gestion événements sportifs (CRUD, RGPD)
...
# Integration Tests: Sport

This folder contains advanced integration tests for all sport-related routes and services (event management, AI, plugins, GDPR, multilingual, logs, accessibility, etc.) in Dihya.

## Goals
- Full coverage of sport APIs (event management, AI, logs, plugins, GDPR, SEO, accessibility, etc.).
...
## Test Structure
- **Unit**: Sport service mocks, schema validation, security, i18n.
- **Integration**: REST/GraphQL endpoints, event management, logs, SEO, accessibility.
...
## Test Examples
- Sport event management (CRUD, GDPR)
