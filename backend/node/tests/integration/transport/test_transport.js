 ...
 * @description Tests d'intégration avancés pour l'API transport (REST/GraphQL, sécurité, i18n, plugins, RGPD, etc.)
 * @author Dihya
 * @date 2025-05-25
 */
...
describe('Transport API - Sécurité, i18n, RGPD, Plugins, SEO', () => {
  it('GET /api/transport retourne la liste (auth, i18n)', async () => {
  ...
  it('POST /api/transport (admin)', async () => {
    const res = await request(app)
      .post('/api/transport')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ nom: 'Transport IA', description: 'Transport automatisé' })
      .expect(201);
