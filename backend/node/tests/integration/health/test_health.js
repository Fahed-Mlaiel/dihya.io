/**
 * Test d'intégration avancé pour le module Health (REST, GraphQL, sécurité, i18n, plugins, RGPD, audit, multitenancy, fallback IA, SEO, multilingue)
...
describe('Health API Integration', () => {
  ...
  it('GET /api/health/records (secured, i18n, audit, plugins)', async () => {
  ...
  it('POST /api/health/records (validation, RGPD, plugins, audit)', async () => {
    const res = await request(app)
      .post('/api/health/records')
      .set('Authorization', `Bearer ${jwt}`)
      .send({ type: 'consultation', user: 'user1', confidential: true });
    expect(res.statusCode).toBe(201);
    expect(res.body).toHaveProperty('id');
    expect(res.body.meta).toHaveProperty('audit');
    expect(res.body.meta).toHaveProperty('rgpd');
  });
  it('GraphQL /api/health/graphql (roles, plugins, fallback IA)', async () => {
    const query = '{ records { id type user } }';
    const res = await request(app)
  ...
  it('Plugin dynamique (mock)', async () => {
    const pluginRes = await mockPlugin('health', { action: 'simulate' });
    expect(pluginRes).toHaveProperty('status', 'ok');
  });
  it('Fallback IA open source', async () => {
    const res = await request(app)
      .post('/api/health/records/ai-fallback')
      .set('Authorization', `Bearer ${jwt}`)
      .send({ prompt: 'simulate health' });
    expect(res.statusCode).toBe(200);
