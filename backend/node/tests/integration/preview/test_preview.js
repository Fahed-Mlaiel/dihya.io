 ...
 * @description Tests d'intégration avancés pour l'API preview (REST/GraphQL, sécurité, i18n, plugins, RGPD, etc.)
 * @author Dihya
 * @date 2025-05-25
 */
...
describe('Preview API - Sécurité, i18n, RGPD, Plugins, SEO', () => {
  it('GET /api/preview retourne la liste (auth, i18n)', async () => {
  ...
  it('POST /api/preview (admin)', async () => {
    const res = await request(app)
      .post('/api/preview')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ nom: 'Preview IA', description: 'Aperçu automatisé' })
      .expect(201);
