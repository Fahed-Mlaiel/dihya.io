/**
 * Test d'intégration avancé pour le module Construction (REST, GraphQL, sécurité, i18n, plugins, RGPD, audit, multitenancy, fallback IA, SEO, multilingue)
...
describe('Construction API Integration', () => {
  ...
  it('GET /api/construction/sites (secured, i18n, audit, plugins)', async () => {
  ...
  it('POST /api/construction/sites (validation, RGPD, plugins, audit)', async () => {
    const res = await request(app)
      .post('/api/construction/sites')
      .set('Authorization', `Bearer ${jwt}`)
      .send({ name: 'Site B', user: 'user1', confidential: true });
    expect(res.statusCode).toBe(201);
    expect(res.body).toHaveProperty('id');
    expect(res.body.meta).toHaveProperty('audit');
    expect(res.body.meta).toHaveProperty('rgpd');
  });
  it('GraphQL /api/construction/graphql (roles, plugins, fallback IA)', async () => {
    const query = '{ sites { id name user } }';
    const res = await request(app)
      .post('/api/construction/graphql')
      .set('Authorization', `Bearer ${jwt}`)
      .send({ query });
    expect(res.statusCode).toBe(200);
  ...
  it('Sécurité CORS, WAF, anti-DDOS', async () => {
    const res = await request(app)
      .options('/api/construction/sites')
      .set('Origin', 'https://evil.com');
    expect(res.headers['access-control-allow-origin']).not.toBe('https://evil.com');
  });
  it('Audit log, anonymisation, export RGPD', async () => {
    const res = await request(app)
      .get('/api/construction/sites/export')
      .set('Authorization', `Bearer ${jwt}`);
    expect(res.statusCode).toBe(200);
  ...
  it('Plugin dynamique (mock)', async () => {
    const pluginRes = await mockPlugin('construction', { action: 'simulate' });
    expect(pluginRes).toHaveProperty('status', 'ok');
  });
  it('Fallback IA open source', async () => {
    const res = await request(app)
      .post('/api/construction/sites/ai-fallback')
      .set('Authorization', `Bearer ${jwt}`)
      .send({ prompt: 'simulate site' });
    expect(res.statusCode).toBe(200);
