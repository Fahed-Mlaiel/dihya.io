/*
 * Test d'intégration ultra avancé pour les routes juridiques
...
describe('Juridique Integration', () => {
  ...
  it('should secure all legal endpoints (CORS, JWT, WAF, anti-DDOS)', async () => {
  ...
  it('should log and audit all legal actions', async () => {
    const res = await request(app)
      .post('/api/legal/audit')
      .set('Authorization', `Bearer ${adminToken}`)
      .set('X-Tenant-Id', tenantId)
      .send({ action: 'export', target: 'user_data' })
      .expect(201);
