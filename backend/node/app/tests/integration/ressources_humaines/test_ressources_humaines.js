/*
 * Test d'intégration ultra avancé pour les routes RH
...
describe('Ressources Humaines Integration', () => {
  ...
  it('should secure all HR endpoints (CORS, JWT, WAF, anti-DDOS)', async () => {
  ...
  it('should log and audit all HR actions', async () => {
    const res = await request(app)
      .post('/api/hr/audit')
      .set('Authorization', `Bearer ${adminToken}`)
      .set('X-Tenant-Id', tenantId)
      .send({ action: 'update', target: 'employee' })
      .expect(201);
