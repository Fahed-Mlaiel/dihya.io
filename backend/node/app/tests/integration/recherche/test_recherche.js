/**
 * Test d'intégration avancé – Recherche (IA/VR/AR)
 ...
 * @module tests/integration/recherche/test_recherche
 */
const request = require('supertest');
const app = require('../../../src/app');
const { getJWT, getTestTenant, getTestUser } = require('../../utils/auth');

describe('Intégration Recherche – Projets IA/VR/AR', () => {
  let jwt, tenant, user;
  beforeAll(async () => {
  ...
  it('crée un projet IA/VR/AR recherche sécurisé et multilingue', async () => {
    const res = await request(app)
      .post(`/api/${tenant.slug}/recherche/projets`)
      .set('Authorization', `Bearer ${jwt}`)
      .set('Accept-Language', 'fr')
      .send({
        nom: 'Projet Recherche IA',
        type: 'IA',
        description: 'Projet recherche avancée',
        langues: ['fr', 'en', 'ar'],
  ...
  it('vérifie la conformité RGPD, accessibilité et SEO backend', async () => {
    const res = await request(app)
      .get(`/api/${tenant.slug}/recherche/projets?audit=true`)
      .set('Authorization', `Bearer ${jwt}`)
  ...
  it('gère le fallback IA (LLaMA/Mixtral/Mistral) et plugins dynamiques', async () => {
    const res = await request(app)
      .post(`/api/${tenant.slug}/recherche/projets/ia-fallback`)
      .set('Authorization', `Bearer ${jwt}`)
      .send({ prompt: 'Génère un projet recherche avancée', fallback: ['llama', 'mixtral'] });
    expect(res.statusCode).toBe(200);
  ...
  it('applique les politiques RBAC et multitenancy', async () => {
    const res = await request(app)
      .get(`/api/${tenant.slug}/recherche/projets`)
      .set('Authorization', `Bearer ${jwt}`);
    expect(res.statusCode).toBe(200);
