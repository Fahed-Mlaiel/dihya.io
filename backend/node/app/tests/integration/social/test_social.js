/*
 * Test d'intégration ultra avancé pour les routes sociales
...
describe('Social Integration', () => {
  ...
  it('should secure all social endpoints (CORS, JWT, WAF, anti-DDOS)', async () => {
  ...
  it('should log and audit all social actions', async () => {
    const res = await request(app)
      .post('/api/social/audit')
      .set('Authorization', `Bearer ${adminToken}`)
      .set('X-Tenant-Id', tenantId)
      .send({ action: 'post', target: 'profile' })
      .expect(201);
