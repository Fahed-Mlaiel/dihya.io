/*
 * Test d'intégration ultra avancé pour les routes restauration
...
describe('Restauration Integration', () => {
  ...
  it('should secure all catering endpoints (CORS, JWT, WAF, anti-DDOS)', async () => {
  ...
  it('should log and audit all catering actions', async () => {
    const res = await request(app)
      .post('/api/catering/audit')
      .set('Authorization', `Bearer ${adminToken}`)
      .set('X-Tenant-Id', tenantId)
      .send({ action: 'book', target: 'menu' })
      .expect(201);
    expect(res.body.audit).toBe(true);
  });

  it('should support AI suggestions for menus', async () => {
    const res = await request(app)
      .post('/api/catering/ai')
      .set('Authorization', `Bearer ${adminToken}`)
      .set('X-Tenant-Id', tenantId)
