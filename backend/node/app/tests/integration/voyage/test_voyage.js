/**
 * Test d'intégration avancé – Voyage (IA/VR/AR)
 ...
 * @module tests/integration/voyage/test_voyage
 */
const request = require('supertest');
const app = require('../../../src/app');
const { getJWT, getTestTenant, getTestUser } = require('../../utils/auth');

describe('Intégration Voyage – Projets IA/VR/AR', () => {
  let jwt, tenant, user;
  beforeAll(async () => {
  ...
  it('crée un projet IA/VR/AR voyage sécurisé et multilingue', async () => {
    const res = await request(app)
      .post(`/api/${tenant.slug}/voyage/projets`)
      .set('Authorization', `Bearer ${jwt}`)
      .set('Accept-Language', 'fr')
      .send({
        nom: 'Projet Voyage IA',
        type: 'AR',
        description: 'Projet voyage immersif',
        langues: ['fr', 'en', 'ar'],
  ...
  it('vérifie la conformité RGPD, accessibilité et SEO backend', async () => {
    const res = await request(app)
      .get(`/api/${tenant.slug}/voyage/projets?audit=true`)
      .set('Authorization', `Bearer ${jwt}`)
  ...
  it('gère le fallback IA (LLaMA/Mixtral/Mistral) et plugins dynamiques', async () => {
    const res = await request(app)
      .post(`/api/${tenant.slug}/voyage/projets/ia-fallback`)
      .set('Authorization', `Bearer ${jwt}`)
      .send({ prompt: 'Génère un projet voyage immersif', fallback: ['llama', 'mixtral'] });
    expect(res.statusCode).toBe(200);
  ...
  it('applique les politiques RBAC et multitenancy', async () => {
    const res = await request(app)
      .get(`/api/${tenant.slug}/voyage/projets`)
      .set('Authorization', `Bearer ${jwt}`);
    expect(res.statusCode).toBe(200);
