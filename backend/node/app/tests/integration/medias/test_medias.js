/**
 * Test d'intégration avancé – Médias (IA/VR/AR)
 ...
 * @module tests/integration/medias/test_medias
 */
const request = require('supertest');
const app = require('../../../src/app');
const { getJWT, getTestTenant, getTestUser } = require('../../utils/auth');

describe('Intégration Médias – Projets IA/VR/AR', () => {
  let jwt, tenant, user;
  beforeAll(async () => {
  ...
  it('crée un projet IA/VR/AR médias sécurisé et multilingue', async () => {
    const res = await request(app)
      .post(`/api/${tenant.slug}/medias/projets`)
      .set('Authorization', `Bearer ${jwt}`)
      .set('Accept-Language', 'fr')
      .send({
        nom: 'Projet Médias IA',
        type: 'AR',
        description: 'Projet médias immersif',
        langues: ['fr', 'en', 'ar'],
  ...
  it('vérifie la conformité RGPD, accessibilité et SEO backend', async () => {
    const res = await request(app)
      .get(`/api/${tenant.slug}/medias/projets?audit=true`)
      .set('Authorization', `Bearer ${jwt}`)
  ...
  it('gère le fallback IA (LLaMA/Mixtral/Mistral) et plugins dynamiques', async () => {
    const res = await request(app)
      .post(`/api/${tenant.slug}/medias/projets/ia-fallback`)
      .set('Authorization', `Bearer ${jwt}`)
      .send({ prompt: 'Génère un projet médias immersif', fallback: ['llama', 'mixtral'] });
    expect(res.statusCode).toBe(200);
  ...
  it('applique les politiques RBAC et multitenancy', async () => {
    const res = await request(app)
      .get(`/api/${tenant.slug}/medias/projets`)
      .set('Authorization', `Bearer ${jwt}`);
    expect(res.statusCode).toBe(200);
