/**
 * Test d'intégration avancé – Construction (IA/VR/AR)
 ...
 * @module tests/integration/construction/test_construction
 */
const request = require('supertest');
const app = require('../../../src/app');
const { getJWT, getTestTenant, getTestUser } = require('../../utils/auth');

describe('Intégration Construction – Projets IA/VR/AR', () => {
  let jwt, tenant, user;
  beforeAll(async () => {
  ...
  it('crée un projet IA/VR/AR construction sécurisé et multilingue', async () => {
    const res = await request(app)
      .post(`/api/${tenant.slug}/construction/projets`)
      .set('Authorization', `Bearer ${jwt}`)
      .set('Accept-Language', 'fr')
      .send({
        nom: 'Projet Construction IA',
        type: 'VR',
        description: 'Projet construction immersif',
        langues: ['fr', 'en', 'ar'],
  ...
  it('vérifie la conformité RGPD, accessibilité et SEO backend', async () => {
    const res = await request(app)
      .get(`/api/${tenant.slug}/construction/projets?audit=true`)
      .set('Authorization', `Bearer ${jwt}`)
  ...
  it('gère le fallback IA (LLaMA/Mixtral/Mistral) et plugins dynamiques', async () => {
    const res = await request(app)
      .post(`/api/${tenant.slug}/construction/projets/ia-fallback`)
      .set('Authorization', `Bearer ${jwt}`)
      .send({ prompt: 'Génère un projet construction immersif', fallback: ['llama', 'mixtral'] });
    expect(res.statusCode).toBe(200);
  ...
  it('applique les politiques RBAC et multitenancy', async () => {
    const res = await request(app)
      .get(`/api/${tenant.slug}/construction/projets`)
      .set('Authorization', `Bearer ${jwt}`);
    expect(res.statusCode).toBe(200);
