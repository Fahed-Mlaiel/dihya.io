/**
 * @file template.js
 * @description Template REST/GraphQL pour projets Medias (IA, VR, AR)
 ...
 * /medias:
 *   get:
 *     summary: Liste des projets medias
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
...
router.get('/',
  ...
  async (req, res) => {
    ...
    res.json({
      message: req.t('medias.list.success'),
      data: [/* ...exemple de projets medias... */]
    });
  }
);
 ...
 * /medias:
 *   post:
 *     summary: Création d'un projet medias
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
...
router.post('/',
  ...
  async (req, res) => {
    ...
    res.status(201).json({
      message: req.t('medias.create.success'),
      id: 'exemple-id'
    });
  }
);
