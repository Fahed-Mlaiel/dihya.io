/**
 * @file template.js
 * @description Template REST/GraphQL pour projets Manufacturing (IA, VR, AR)
 ...
 * /manufacturing:
 *   get:
 *     summary: Liste des projets manufacturing
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
...
router.get('/',
  ...
  async (req, res) => {
    ...
    res.json({
      message: req.t('manufacturing.list.success'),
      data: [/* ...exemple de projets manufacturing... */]
    });
  }
);
 ...
 * /manufacturing:
 *   post:
 *     summary: Création d'un projet manufacturing
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
...
router.post('/',
  ...
  async (req, res) => {
    ...
    res.status(201).json({
      message: req.t('manufacturing.create.success'),
      id: 'exemple-id'
    });
  }
);
