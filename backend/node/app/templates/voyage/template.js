/**
 * @file template.js
 * @description Template avancé pour la gestion de projets "voyage" (web, mobile, IA, VR, AR) dans Dihya Coding.
 * @author Dihya Team
 * @version 1.0.0
 * @license MIT
 *
...
const { createVoyage, listVoyages } = require('../../../../services/voyageService');

/**
 * @swagger
 * /api/voyage:
 *   post:
 *     summary: Créer un projet voyage
 *     tags: [Voyage]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const project = await createVoyage({ name, description, lang, tenant });
...
router.get(
  ...
  async (req, res) => {
    try {
      const voyages = await listVoyages(req.user, req.tenant);
