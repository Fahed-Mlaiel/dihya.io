/**
 * @file template.js
 * @description Template REST/GraphQL pour projets Marketing (IA, VR, AR)
 ...
 * /marketing:
 *   get:
 *     summary: Liste des projets marketing
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
...
router.get('/',
  ...
  async (req, res) => {
    ...
    res.json({
      message: req.t('marketing.list.success'),
      data: [/* ...exemple de projets marketing... */]
    });
  }
);
 ...
 * /marketing:
 *   post:
 *     summary: Création d'un projet marketing
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
...
router.post('/',
  ...
  async (req, res) => {
    ...
    res.status(201).json({
      message: req.t('marketing.create.success'),
      id: 'exemple-id'
    });
  }
);
