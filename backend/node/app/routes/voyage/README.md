...
## Description
Gestion voyage avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /voyage` : Liste voyage
- `POST /voyage` : Créer une entrée
- `PUT /voyage/:id` : Modifier
- `DELETE /voyage/:id` : Supprimer
- `POST /voyage/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/voyage
```
