...
## Description
Gestion voice avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /voice` : Liste voice
- `POST /voice` : Créer une entrée
- `PUT /voice/:id` : Modifier
- `DELETE /voice/:id` : Supprimer
- `POST /voice/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/voice
```
