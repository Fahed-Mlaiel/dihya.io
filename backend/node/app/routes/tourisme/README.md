...
## Description
Gestion tourisme avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /tourisme` : Liste tourisme
- `POST /tourisme` : Créer une entrée
- `PUT /tourisme/:id` : Modifier
- `DELETE /tourisme/:id` : Supprimer
- `POST /tourisme/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/tourisme
```
