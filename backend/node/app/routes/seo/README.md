...
## Description
Gestion SEO avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /seo` : Liste SEO
- `POST /seo` : Créer une entrée
- `PUT /seo/:id` : Modifier
- `DELETE /seo/:id` : Supprimer
- `POST /seo/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/seo
```
