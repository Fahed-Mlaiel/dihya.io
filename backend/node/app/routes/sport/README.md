...
## Description
Gestion sport avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /sport` : Liste sport
- `POST /sport` : Créer une entrée
- `PUT /sport/:id` : Modifier
- `DELETE /sport/:id` : Supprimer
- `POST /sport/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/sport
```
