...
## Description
Gestion preview avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /preview` : Liste preview
- `POST /preview` : Créer une entrée
- `PUT /preview/:id` : Modifier
- `DELETE /preview/:id` : Supprimer
- `POST /preview/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/preview
```
