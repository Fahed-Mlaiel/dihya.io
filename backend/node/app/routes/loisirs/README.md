...
## Description
Gestion loisirs avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /loisirs` : Liste loisirs
- `POST /loisirs` : Créer une entrée
- `PUT /loisirs/:id` : Modifier
- `DELETE /loisirs/:id` : Supprimer
- `POST /loisirs/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/loisirs
```
