/**
 * @fileoverview Routes loisirs avancées pour la gestion de projets IA, VR, AR, etc.
...
router.use(auditMiddleware('loisirs'));
router.use(seoMiddleware());
router.use(rgpdMiddleware());
router.use(pluginMiddleware('loisirs'));

router.get('/', checkJwt, checkRole(['admin', 'user', 'invite']), (req, res) => {
  res.status(200).json({ success: true, data: [{ id: 1, type: req.t('evenement'), statut: 'ouvert' }] });
});
...
router.get('/audit-log', checkJwt, checkRole(['admin']), auditMiddleware('loisirs'), (req, res) => {
  res.json({ audit: [] });
});
