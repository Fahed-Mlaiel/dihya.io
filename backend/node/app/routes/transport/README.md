...
## Description
Gestion transport avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /transport` : Liste transport
- `POST /transport` : Créer une entrée
- `PUT /transport/:id` : Modifier
- `DELETE /transport/:id` : Supprimer
- `POST /transport/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/transport
```
