...
## Description
Gestion médias avancée pour projets IA, VR, AR, etc. Sécurité maximale, multitenant, plugins, RGPD, audit, SEO, IA fallback.
...
- `GET /medias` : Liste médias
- `POST /medias` : Créer une entrée
- `PUT /medias/:id` : Modifier
- `DELETE /medias/:id` : Supprimer
- `POST /medias/plugin` : Plugins dynamiques

## Exemple d'appel
```bash
curl -H "Authorization: Bearer <token>" https://<host>/medias
```
