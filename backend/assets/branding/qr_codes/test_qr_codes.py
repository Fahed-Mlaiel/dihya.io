"""
test_qr_codes.py – CI/CD : tests QR codes backend (lint SVG/PNG, accessibilité, hash, RGPD)
"""
import os
import json
import hashlib
...
def main():
    ...
    for fname in os.listdir(DIR):
        if fname.endswith((".svg", ".json", ".png")):
            print_hash(os.path.join(DIR, fname))
    print("Tous les tests de QR codes sont passés.")
