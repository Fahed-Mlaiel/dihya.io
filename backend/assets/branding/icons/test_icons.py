"""
test_icons.py – CI/CD : tests automatiques SVG/PNG (accessibilité, RGPD, hash, lint)
"""
import os
import json
import hashlib
...
def check_metadata(svg_path):
    meta = os.path.join(ICONS_DIR, f"meta_{os.path.basename(svg_path)[:-4]}.json")
...
def main():
    ...
    for fname in os.listdir(ICONS_DIR):
        if fname.endswith(".svg") or fname.endswith(".json"):
            print_hash(os.path.join(ICONS_DIR, fname))
    print("Tous les tests d’icônes sont passés.")
