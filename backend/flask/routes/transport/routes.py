"""
routes.py - Routes RESTful/GraphQL pour la gestion de projets transport
...
bp = Blueprint('routes_transport', __name__, url_prefix='/api/transport')
...
def list_projects_transport() -> Any:
    """Lister les projets transport (multitenancy, i18n, audit, plugins, fallback IA, RGPD, export, anonymisation, accessibilité, SEO).
    Returns:
        JSON: Liste des projets transport filtrés selon tenant, rôle, langue, plugins, fallback IA, RGPD, accessibilité, SEO.
    """
    ...
    audit_log(user, 'list_projects_transport', tenant, locale)
...
def create_project_transport() -> Any:
    """Créer un projet transport (validation, audit, plugins, RGPD, accessibilité, SEO).
    Returns:
        JSON: Projet transport créé ou erreur de validation.
    """
    user = get_jwt_identity()
    tenant = get_tenant(user)
    locale = get_locale(request)
    ...
    audit_log(user, 'create_project_transport', tenant, locale, project)
    return jsonify({'msg': _('Projet transport créé'), 'project': project}), 201
...
@jwt_required()
@rbac_required(['admin', 'user', 'invité'])
def graphql_transport() -> Any:
    """Endpoint GraphQL transport (sécurité, plugins, audit, RGPD, SEO, accessibilité)."""
    user = get_jwt_identity()
    tenant = get_tenant(user)
    locale = get_locale(request)
    data = request.get_json()
    result = graphql_ia_schema.execute(data.get('query'), context_value={'user': user, 'tenant': tenant, 'locale': locale})
    audit_log(user, 'graphql_transport', tenant, locale, data)
