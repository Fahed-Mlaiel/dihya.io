...
def list_projects_blockchain() -> Any:
    """Lister les projets blockchain (multitenancy, i18n, audit, plugins, fallback IA, RGPD, export, anonymisation, accessibilité, SEO).
    Returns:
        JSON: Liste des projets blockchain filtrés selon tenant, rôle, langue, plugins, fallback IA, RGPD, accessibilité, SEO.
    """
    ...
    audit_log(user, 'list_projects_blockchain', tenant, locale)
...
def create_project_blockchain() -> Any:
    """Créer un projet blockchain (validation, audit, plugins, RGPD, accessibilité, SEO).
    Returns:
        JSON: Projet blockchain créé ou erreur de validation.
    """
    user = get_jwt_identity()
    tenant = get_tenant(user)
    locale = get_locale(request)
    ...
    audit_log(user, 'create_project_blockchain', tenant, locale, project)
    return jsonify({'msg': _('Projet blockchain créé'), 'project': project}), 201
...
@jwt_required()
@rbac_required(['admin', 'user', 'invité'])
def graphql_blockchain() -> Any:
    """Endpoint GraphQL blockchain (sécurité, plugins, audit, RGPD, SEO, accessibilité)."""
    user = get_jwt_identity()
    tenant = get_tenant(user)
    locale = get_locale(request)
    data = request.get_json()
    result = graphql_ia_schema.execute(data.get('query'), context_value={'user': user, 'tenant': tenant, 'locale': locale})
    audit_log(user, 'graphql_blockchain', tenant, locale, data)
