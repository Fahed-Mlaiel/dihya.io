...
def list_projects_3d() -> Any:
    """Lister les projets 3D (multitenancy, i18n, audit, plugins, fallback IA, RGPD, export, anonymisation, accessibilité, SEO).
    Returns:
        JSON: Liste des projets 3D filtrés selon tenant, rôle, langue, plugins, fallback IA, RGPD, accessibilité, SEO.
    """
    ...
    audit_log(user, 'list_projects_3d', tenant, locale)
...
def create_project_3d() -> Any:
    """Créer un projet 3D (validation, audit, plugins, RGPD, accessibilité, SEO).
    Returns:
        JSON: Projet 3D créé ou erreur de validation.
    """
    user = get_jwt_identity()
    tenant = get_tenant(user)
    locale = get_locale(request)
    ...
    audit_log(user, 'create_project_3d', tenant, locale, project)
    return jsonify({'msg': _('Projet 3D créé'), 'project': project}), 201
...
@rbac_required(['admin', 'user', 'invité'])
def graphql_3d() -> Any:
    """Endpoint GraphQL 3D (sécurité, plugins, audit, RGPD, SEO, accessibilité)."""
    user = get_jwt_identity()
    tenant = get_tenant(user)
    locale = get_locale(request)
    data = request.get_json()
    result = graphql_ia_schema.execute(data.get('query'), context_value={'user': user, 'tenant': tenant, 'locale': locale})
    audit_log(user, 'graphql_3d', tenant, locale, data)
