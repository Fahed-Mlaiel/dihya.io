"""
Schemas avancés pour le module blockchain (validation, sérialisation, RGPD, multitenancy, sécurité).
"""
from marshmallow import Schema, fields, validate

class BlockchainBaseSchema(Schema):
...
class BlockchainCreateSchema(BlockchainBaseSchema):
    pass

class BlockchainUpdateSchema(Schema):
    nom = fields.Str(validate=validate.Length(min=1, max=100))
    type = fields.Str(validate=validate.Length(min=1, max=50))
    pays = fields.Str(validate=validate.Length(max=56))

class BlockchainRGPDExportSchema(Schema):
    id = fields.Int()
    nom = fields.Str()
    type = fields.Str()
    pays = fields.Str()
