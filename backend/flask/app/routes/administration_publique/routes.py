"""
Routes RESTful & GraphQL pour l’administration publique (IA, VR, AR, etc.)
...
bp = Blueprint('administration_publique', __name__, url_prefix='/api/administration_publique')

@bp.route('/projects', methods=['GET'])
@jwt_required()
# ...@require_role(['admin', 'user'])...
def list_projects():
    """Liste les projets d’administration publique (multilingue, sécurisé, audité)."""
...
# ...@require_role(['admin'])...
def create_project():
    """Crée un projet (validation, RGPD, fallback IA, audit)."""
    data = request.json
