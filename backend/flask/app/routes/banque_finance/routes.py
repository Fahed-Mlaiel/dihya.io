"""
Routes RESTful & GraphQL pour la banque et la finance (IA, VR, AR, etc.)
...
bp = Blueprint('banque_finance', __name__, url_prefix='/api/banque_finance')

@bp.route('/projects', methods=['GET'])
@jwt_required()
# ...@require_role(['admin', 'user'])...
def list_projects():
    """Liste les projets bancaires et financiers (multilingue, sécurisé, audité)."""
...
# ...@require_role(['admin'])...
def create_project():
    """Crée un projet bancaire/financier (validation, RGPD, fallback IA, audit)."""
    data = request.json
