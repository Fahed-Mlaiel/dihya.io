"""
Schemas avancés pour le module construction (validation, sérialisation, RGPD, multitenancy, sécurité).
"""
from marshmallow import Schema, fields, validate

class ConstructionBaseSchema(Schema):
...
class ConstructionCreateSchema(ConstructionBaseSchema):
    pass

class ConstructionUpdateSchema(Schema):
    nom = fields.Str(validate=validate.Length(min=1, max=100))
    type = fields.Str(validate=validate.Length(min=1, max=50))
    pays = fields.Str(validate=validate.Length(max=56))

class ConstructionRGPDExportSchema(Schema):
    id = fields.Int()
    nom = fields.Str()
    type = fields.Str()
    pays = fields.Str()
