...
# --- CODE PYTHON FLASK POUR TEMPLATE MÉDIAS ---
from flask import Blueprint, request, jsonify
from flask_jwt_extended import jwt_required, get_jwt_identity

medias_bp = Blueprint('medias', __name__)

@medias_bp.route('/api/articles', methods=['GET'])
@jwt_required()
def list_articles():
    """Liste des articles/contenus (mock)."""
    return jsonify([{"id": 1, "title": "Exemple d'article", "status": "publié"}])

@medias_bp.route('/api/articles', methods=['POST'])
@jwt_required()
def create_article():
    """Créer/modifier un article (mock)."""
    data = request.json
    return jsonify({"id": 2, "title": data.get("title", "Nouveau"), "status": "brouillon"}), 201

# ... Ajoutez ici les autres endpoints RESTful mockés pour la couverture ...
# --- FIN CODE PYTHON FLASK ---
