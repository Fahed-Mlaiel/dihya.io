"""
Utilitaires métier Services à la Personne – Dihya Coding
...
def validate_service_payload(data: Dict[str, Any]) -> None:
    """Valide le payload service à la personne (type hints, docstring, RGPD)."""
    if 'type' not in data or 'date' not in data:
        raise ValueError(translate('Données service invalides', get_locale()))
