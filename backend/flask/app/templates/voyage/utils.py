"""
Utilitaires métier Voyage – Dihya Coding
...
def validate_trip_payload(data: Dict[str, Any]) -> None:
    """Valide le payload voyage (type hints, docstring, RGPD)."""
    if 'destination' not in data or 'date' not in data:
        raise ValueError(translate('Données voyage invalides', get_locale()))
