"""
Tests unitaires pour la gestion des webhooks CI/CD (app.webhooks.cicd_webhooks) — Dihya Coding.
...
from backend.flask.app.webhooks import cicd_webhooks

def make_app():
    """Crée une app Flask de test avec le blueprint cicd_webhook."""
    app = Flask(__name__)
    app.register_blueprint(cicd_webhooks.cicd_webhook_blueprint, url_prefix="/webhooks/cicd")
    return app
...
def client(monkeypatch):
    ...
    app = make_app()
    secret = "cicd_secret"
    monkeypatch.setenv("CICD_WEBHOOK_SECRET", secret)
    with app.test_client() as client:
        yield client

def test_webhook_valid_signature(client):
    """Test réception d'un webhook CI/CD avec signature valide."""
    payload = b'{"event":"build_completed","repository":"repo1","branch":"main","status":"success"}'
    secret = os.environ["CICD_WEBHOOK_SECRET"]
...
def test_webhook_invalid_signature(client):
    """Test rejet d'un webhook CI/CD avec signature invalide."""
    payload = b'{"event":"build_completed","repository":"repo1","branch":"main","status":"success"}'
    resp = client.post(
        "/webhooks/cicd/",
        data=payload,
...
def test_webhook_no_secret(monkeypatch):
    ...
    app = make_app()
    monkeypatch.delenv("CICD_WEBHOOK_SECRET", raising=False)
    with app.test_client() as client:
        payload = b'{"event":"build_completed","repository":"repo1","branch":"main","status":"success"}'
        resp = client.post(
            "/webhooks/cicd/",
