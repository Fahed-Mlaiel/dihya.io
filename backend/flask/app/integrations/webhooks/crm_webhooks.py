...
def handle_crm_webhook(payload, context):
    """
    Point d'entrée principal pour le traitement des webhooks CRM (POST).
...
def flask_crm_webhook():
    ...
    try:
        ...
        result = handle_crm_webhook(payload, context)
...
bp.add_url_rule("/", view_func=flask_crm_webhook, methods=["POST"])

# À intégrer dans votre app Flask principale :
# from backend.flask.app.integrations.webhooks.crm_webhooks import bp as crm_webhooks_bp
