# Social – Dihya Integration Test Suite
...
## Présentation
Ce module fournit des tests d'intégration avancés pour la gestion de projets social, réseaux, IA, VR, AR dans Dihya. Il couvre la sécurité (CORS, JWT, WAF, anti-DDOS), l'audit, la conformité RGPD, la génération de routes REST/GraphQL, la gestion multitenant, l'intégration IA (LLaMA, Mixtral, Mistral), et la documentation multilingue.

## Fonctionnalités principales
- Génération automatique de routes RESTful & GraphQL pour social
...
- `/api/social/networks` (CRUD, multilingue, RBAC, audit)
- `/api/social/posts` (gestion, export, audit)
- `/api/social/plugins` (ajout dynamique, sécurité)
- `/graphql` (requêtes avancées, rôles, fallback IA)

## Exécution des tests
```bash
pytest --cov=social --maxfail=1 --disable-warnings
```
