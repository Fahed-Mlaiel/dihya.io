# Publicité – Dihya Integration Test Suite
...
- `/api/publicite/campaigns` (CRUD, multilingue, RBAC, audit)
- `/api/publicite/ads` (gestion, export, audit)
- `/api/publicite/plugins` (ajout dynamique, sécurité)
- `/graphql` (requêtes avancées, rôles, fallback IA)

## Exécution des tests
```bash
pytest --cov=publicite --maxfail=1 --disable-warnings
```
