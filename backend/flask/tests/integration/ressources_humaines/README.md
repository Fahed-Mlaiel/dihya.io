# Ressources Humaines – Dihya Integration Test Suite
...
## Présentation
Ce module fournit des tests d'intégration avancés pour la gestion de projets RH, talents, IA, VR, AR dans Dihya. Il couvre la sécurité (CORS, JWT, WAF, anti-DDOS), l'audit, la conformité RGPD, la génération de routes REST/GraphQL, la gestion multitenant, l'intégration IA (LLaMA, Mixtral, Mistral), et la documentation multilingue.

## Fonctionnalités principales
- Génération automatique de routes RESTful & GraphQL pour RH
...
- `/api/ressources_humaines/employees` (CRUD, multilingue, RBAC, audit)
- `/api/ressources_humaines/recruitment` (gestion, export, audit)
- `/api/ressources_humaines/plugins` (ajout dynamique, sécurité)
- `/graphql` (requêtes avancées, rôles, fallback IA)

## Exécution des tests
```bash
pytest --cov=ressources_humaines --maxfail=1 --disable-warnings
```
