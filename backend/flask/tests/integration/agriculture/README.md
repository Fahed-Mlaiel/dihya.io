...
## Extension DWeb/IPFS
- Export des logs critiques et rapports de tests agriculture sur IPFS ou stockage décentralisé
...
```yaml
- name: Tests intégration agriculture
  run: pytest backend/flask/tests/integration/agriculture/ --maxfail=1 --disable-warnings --cov=.
```
