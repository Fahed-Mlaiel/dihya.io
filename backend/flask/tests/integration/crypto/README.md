...
- [ ] Multitenancy, gestion des rôles (admin, trader, client, invité)
...
- [ ] Plugins/extensions crypto (audit accès wallet, anonymisation, reporting)
...
## Extension DWeb/IPFS
- Export des logs critiques et rapports de tests crypto sur IPFS ou stockage décentralisé
...
```yaml
- name: Tests intégration crypto
  run: pytest backend/flask/tests/integration/crypto/ --maxfail=1 --disable-warnings --cov=.
```
