# Loisirs – Dihya Integration Test Suite
...
## Présentation
Ce module fournit des tests d'intégration avancés pour la gestion de projets loisirs, culture, IA, VR, AR dans Dihya. Il couvre la sécurité (CORS, JWT, WAF, anti-DDOS), l'audit, la conformité RGPD, la génération de routes REST/GraphQL, la gestion multitenant, l'intégration IA (LLaMA, Mixtral, Mistral), et la documentation multilingue.

## Fonctionnalités principales
- Génération automatique de routes RESTful & GraphQL pour loisirs
...
- `/api/loisirs/events` (CRUD, multilingue, RBAC, audit)
- `/api/loisirs/activities` (gestion, export, audit)
- `/api/loisirs/plugins` (ajout dynamique, sécurité)
- `/graphql` (requêtes avancées, rôles, fallback IA)

## Exécution des tests
```bash
pytest --cov=loisirs --maxfail=1 --disable-warnings
```
