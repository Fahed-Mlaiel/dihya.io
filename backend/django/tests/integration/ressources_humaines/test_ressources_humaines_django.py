"""
Test d'intégration avancé pour la gestion Ressources Humaines dans Dihya.
...
def test_ressources_humaines_routes_security_i18n(api_client):
    url = reverse('ressources-humaines-list')
    ...
    ia_url = reverse('ressources-humaines-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère un process RH'}, format='json')
    ...
    plugin_url = reverse('ressources-humaines-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('ressources-humaines-export')
