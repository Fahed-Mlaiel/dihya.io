"""
Test d'intégration avancé pour la gestion Publicité dans Dihya.
...
def test_publicite_routes_security_i18n(api_client):
    url = reverse('publicite-list')
    ...
    ia_url = reverse('publicite-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère une pub'}, format='json')
    ...
    plugin_url = reverse('publicite-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('publicite-export')
