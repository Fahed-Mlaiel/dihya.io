"""
Tests d'intégration avancés pour le module IT & DevOps de Dihya.
...
def test_it_devops_crud(api_client, admin_user):
    api_client.force_authenticate(user=admin_user)
    resp = api_client.post(reverse('itdevops-list'), {"title": "CI/CD Pipeline", "lang": "fr"}, format='json')
    assert resp.status_code == 201
    resp = api_client.get(reverse('itdevops-list'))
