"""
Test d'intégration avancé pour la gestion Manufacturing dans Dihya.
...
def test_manufacturing_routes_security_i18n(api_client):
    ...
    ia_url = reverse('manufacturing-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère un process'}, format='json')
    ...
    plugin_url = reverse('manufacturing-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('manufacturing-export')
