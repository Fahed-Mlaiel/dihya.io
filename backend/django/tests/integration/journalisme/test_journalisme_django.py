"""
Tests d'intégration avancés pour le module Journalisme de Dihya.
...
def test_journalisme_crud(api_client, admin_user):
    api_client.force_authenticate(user=admin_user)
    resp = api_client.post(reverse('journalisme-list'), {"title": "Article IA", "lang": "fr"}, format='json')
    assert resp.status_code == 201
    resp = api_client.get(reverse('journalisme-list'))
