"""
Test d'intégration avancé pour la gestion Preview dans Dihya.
...
def test_preview_routes_security_i18n(api_client):
    ...
    ia_url = reverse('preview-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère un aperçu'}, format='json')
    ...
    plugin_url = reverse('preview-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('preview-export')
