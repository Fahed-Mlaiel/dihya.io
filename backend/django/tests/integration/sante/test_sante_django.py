"""
Test d'intégration avancé pour la gestion Santé dans Dihya.
...
def test_sante_routes_security_i18n(api_client):
    url = reverse('sante-list')
    ...
    ia_url = reverse('sante-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère un conseil santé'}, format='json')
    ...
    plugin_url = reverse('sante-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('sante-export')
