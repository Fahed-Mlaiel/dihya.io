...
def test_industrie_crud(api_client, admin_user):
    api_client.force_authenticate(user=admin_user)
    resp = api_client.post(reverse('industrie-list'), {"title": "Usine 4.0", "lang": "fr"}, format='json')
    assert resp.status_code == 201
    resp = api_client.get(reverse('industrie-list'))
