"""
Tests d'intégration avancés pour le module Énergie de Dihya.
...
def test_energie_crud(api_client, admin_user):
    api_client.force_authenticate(user=admin_user)
    resp = api_client.post(reverse('energie-list'), {"title": "Projet Solaire", "lang": "fr"}, format='json')
    assert resp.status_code == 201
    resp = api_client.get(reverse('energie-list'))
