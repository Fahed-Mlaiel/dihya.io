"""
Tests d'intégration avancés pour le module Santé de Dihya.
...
def test_health_crud(api_client, admin_user):
    api_client.force_authenticate(user=admin_user)
    resp = api_client.post(reverse('health-list'), {"title": "Dossier Médical", "lang": "fr"}, format='json')
    assert resp.status_code == 201
    resp = api_client.get(reverse('health-list'))
