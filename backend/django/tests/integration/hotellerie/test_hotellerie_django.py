"""
Tests d'intégration avancés pour le module Hôtellerie de Dihya.
...
def test_hotellerie_crud(api_client, admin_user):
    api_client.force_authenticate(user=admin_user)
    resp = api_client.post(reverse('hotellerie-list'), {"title": "Réservation Hôtel", "lang": "fr"}, format='json')
    assert resp.status_code == 201
    resp = api_client.get(reverse('hotellerie-list'))
