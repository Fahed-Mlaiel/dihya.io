...
def test_juridique_crud(api_client, admin_user):
    api_client.force_authenticate(user=admin_user)
    resp = api_client.post(reverse('juridique-list'), {"title": "Contrat RGPD", "lang": "fr"}, format='json')
    assert resp.status_code == 201
    resp = api_client.get(reverse('juridique-list'))
