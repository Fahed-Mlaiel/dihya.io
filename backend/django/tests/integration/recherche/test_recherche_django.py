"""
Test d'intégration avancé pour la gestion Recherche dans Dihya.
...
def test_recherche_routes_security_i18n(api_client):
    url = reverse('recherche-list')
    ...
    ia_url = reverse('recherche-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère une recherche'}, format='json')
    ...
    plugin_url = reverse('recherche-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('recherche-export')
