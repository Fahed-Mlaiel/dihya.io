"""
Tests d'intégration avancés pour le module Gaming de Dihya.
...
def test_gamer_crud(api_client, admin_user):
    api_client.force_authenticate(user=admin_user)
    resp = api_client.post(reverse('gamer-list'), {"title": "Jeu VR", "lang": "fr"}, format='json')
    assert resp.status_code == 201
    resp = api_client.get(reverse('gamer-list'))
