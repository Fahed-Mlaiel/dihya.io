"""
Test d'intégration avancé pour la gestion Science dans Dihya.
...
def test_science_routes_security_i18n(api_client):
    url = reverse('science-list')
    ...
    ia_url = reverse('science-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère une expérience'}, format='json')
    ...
    plugin_url = reverse('science-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('science-export')
