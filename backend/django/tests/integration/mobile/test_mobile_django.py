"""
Test d'intégration avancé pour la gestion Mobile dans Dihya.
...
def test_mobile_routes_security_i18n(api_client):
    ...
    ia_url = reverse('mobile-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère une app mobile'}, format='json')
    ...
    plugin_url = reverse('mobile-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('mobile-export')
