"""
Test d'intégration avancé pour la gestion Restauration dans Dihya.
...
def test_restauration_routes_security_i18n(api_client):
    url = reverse('restauration-list')
    ...
    ia_url = reverse('restauration-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère un menu'}, format='json')
    ...
    plugin_url = reverse('restauration-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('restauration-export')
