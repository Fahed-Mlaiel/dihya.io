"""
Test d'intégration avancé pour la gestion Médias dans Dihya.
...
def test_medias_routes_security_i18n(api_client):
    url = reverse('medias-list')
    ...
    ia_url = reverse('medias-ia-generate')
    response = api_client.post(ia_url, {'prompt': 'Génère un média'}, format='json')
    ...
    plugin_url = reverse('medias-plugin-action')
    response = api_client.post(plugin_url, {'action': 'custom'}, format='json')
    assert response.status_code in [200, 501]

    export_url = reverse('medias-export')
