# 🛡️ Dihya – Politique d’Utilisation & Sécurité des Templates Assurance
...
Garantir la sécurité, la souveraineté, la conformité et la qualité des templates métiers utilisés pour l’assurance (contrats, sinistres, attestations, workflows, IA, plugins) dans la plateforme Dihya.
...
- **Formats acceptés** : .json, .yaml, .yml, .py, .md, .pdf (contrats, attestations, scripts de génération uniquement)
...
- **Tests d’intégrité** : chaque template doit passer les scripts de validation fournis (`validate_assurance_template.py`)
...
- **Séparer** les templates par usage (contrats, sinistres, attestations, workflows, scripts)
