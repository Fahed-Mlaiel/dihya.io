"""
Dihya Hotellerie Template – Tests avancés
=========================================

Tests unitaires, d’intégration et E2E pour les utilitaires de templates du module hotellerie.
...
from .template import (
    render_hotellerie_template,
    validate_hotellerie_template,
...
def test_render_hotellerie_template_ok(lang, example_context):
    html = render_hotellerie_template("example.html", context=example_context, lang=lang)
    assert "<html" in html
    assert "Bienvenue" in html or "Welcome" in html or "مرحبا" in html or "ⴰⵏⴰⵡ" in html

@pytest.mark.parametrize("lang", ["fr", "en", "ar", "tz"])
def test_render_hotellerie_template_user_roles(lang, user_admin, user_manager, user_client, example_context):
    html_admin = render_hotellerie_template("example.html", context=example_context, user=user_admin, lang=lang)
    html_manager = render_hotellerie_template("example.html", context=example_context, user=user_manager, lang=lang)
    html_client = render_hotellerie_template("example.html", context=example_context, user=user_client, lang=lang)
    assert "Administrateur" in html_admin or "Admin" in html_admin or "مشرف" in html_admin or "ⴰⵎⴰⵏⴰⵡⴰⵏ" in html_admin
    assert "Gestionnaire" in html_manager or "Manager" in html_manager or "مدير" in html_manager or "ⴰⵎⴻⴽⴽⴰⵏ" in html_manager
    assert "Client" in html_client or "Customer" in html_client or "عميل" in html_client or "ⴰⵙⵙⴰⵏⴻⵏ" in html_client

def test_render_hotellerie_template_not_exist():
    with pytest.raises(TemplateDoesNotExist):
        render_hotellerie_template("notfound.html")

@pytest.mark.parametrize("lang", ["fr", "en", "ar", "tz"])
def test_validate_hotellerie_template_compliance(lang, example_context):
    html = render_hotellerie_template("example.html", context=example_context, lang=lang)
    results = validate_hotellerie_template(html, lang=lang)
...
def test_validate_hotellerie_template_security_fail():
    html = "<html lang='fr'><body><script>alert('xss')</script></body></html>"
    results = validate_hotellerie_template(html, lang="fr")
    assert results["security"] is False
    assert any("Script inline" in str(e) for e in results["errors"])

def test_validate_hotellerie_template_accessibility_fail():
    html = "<html><body></body></html>"
    results = validate_hotellerie_template(html, lang="fr")
    assert results["accessibility"] is False
    assert any("lang" in str(e) for e in results["errors"])

def test_validate_hotellerie_template_privacy_fail():
    html = "<html lang='fr'><body>{{ client.credit_card }}</body></html>"
    results = validate_hotellerie_template(html, lang="fr")
    assert results["privacy"] is False
    assert any("client" in str(e) or "sensibles" in str(e) for e in results["errors"])
...
def test_end_to_end_template_render_and_validate(lang, user_client, example_context):
    html = render_hotellerie_template("example.html", context=example_context, user=user_client, lang=lang)
    results = validate_hotellerie_template(html, lang=lang)
...
def test_template_env_var_disable_check(monkeypatch):
    ...
    import importlib
    import sys
    mod = importlib.import_module("Dihya.backend.django.app.templates.hotellerie.__init__")
...
def test_template_render_with_missing_context_keys(user_client):
    ...
    html = render_hotellerie_template("example.html", context={}, user=user_client, lang="fr")
    assert "<html" in html
...
def test_template_render_with_special_characters(user_client, example_context):
    ...
    html = render_hotellerie_template("example.html", context=example_context, user=user_client, lang="fr")
