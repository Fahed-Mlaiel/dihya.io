"""
Tests ultra avancés pour le module Dihya E-commerce Templates
-------------------------------------------------------------
...
import sys
sys.path.insert(0, '.')
from backend.django.app.templates.ecommerce.template import EcommerceTemplate, SUPPORTED_LANGS
import pytest
...
def test_template_to_dict_and_from_dict():
    tpl = EcommerceTemplate(
        template_type="paiement",
        lang="en",
        structure=[{"champ": "amount", "label": "Amount"}],
        meta={"roles": ["admin"]}
    )
    d = tpl.to_dict()
    tpl2 = EcommerceTemplate.from_dict(d)
...
def test_template_structure_validation():
    ...
    assert EcommerceTemplate.validate_structure(valid_structure)
    assert not EcommerceTemplate.validate_structure(invalid_structure)
...
def test_license_and_roles_in_meta():
    ...
    assert set(meta["roles"]) >= {"admin", "client"}
