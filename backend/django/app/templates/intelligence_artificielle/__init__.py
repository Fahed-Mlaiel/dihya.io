...
FR : Ce package contient les templates HTML du module "intelligence_artificielle" de Dihya.
EN : This package contains the HTML templates for the "intelligence_artificielle" (AI) module of Dihya.
AR : يحتوي هذا المجلد على قوالب HTML لوحدة "الذكاء الاصطناعي" في ديهيا.
TZ : Asgwas agi yegber templates n HTML n uγmis "intelligence_artificielle" n Dihya.

Fonctionnalités :
- Documentation multilingue (fr, en, ar, amazigh)
- Vérification automatique d’accessibilité, i18n, sécurité à l’import
- Prêt pour extension (Jinja2, plugins, hooks, IA open source)
- Souveraineté numérique : aucune dépendance cloud, sécurité renforcée, fallback IA local

Utilisation :
- Placez vos fichiers .html ici.
- Les templates sont chargés automatiquement par Django.
- Pour hooks avancés, IA, ou extensions, éditez ce fichier.
...
def check_templates():
    ...
    - Pas de fuite de données IA/confidentielles
    """
    errors = []
    for tpl_path in glob.glob(os.path.join(TEMPLATE_DIR, "*.html")):
        with open(tpl_path, encoding="utf-8") as f:
            ...
            if "api_key" in content or "secret" in content or "confidential" in content:
                errors.append(f"{tpl_path}: Potential confidential/AI data leak")
    if errors:
        raise ImportError(
...
# Extension possible : hooks IA, moteurs alternatifs, plugins, fallback open source, etc.
