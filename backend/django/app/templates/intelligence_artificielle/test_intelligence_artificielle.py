"""
Dihya Intelligence Artificielle Template – Tests avancés
========================================================

Tests unitaires, d’intégration et E2E pour les utilitaires de templates du module intelligence_artificielle.
...
@pytest.fixture
def user_datascientist(db):
    user = User.objects.create_user(username="datascientist", password="dspass")
    user.groups.create(name="Data Scientist")
    return user
...
def test_render_ia_template_ok(lang, example_context):
    html = render_ia_template("example.html", context=example_context, lang=lang)
    assert "<html" in html
    assert "Bienvenue" in html or "Welcome" in html or "مرحبا" in html or "ⴰⵏⴰⵡ" in html

@pytest.mark.parametrize("lang", ["fr", "en", "ar", "tz"])
def test_render_ia_template_user_roles(lang, user_admin, user_datascientist, user_operator, user_client, example_context):
    html_admin = render_ia_template("example.html", context=example_context, user=user_admin, lang=lang)
    html_ds = render_ia_template("example.html", context=example_context, user=user_datascientist, lang=lang)
    html_operator = render_ia_template("example.html", context=example_context, user=user_operator, lang=lang)
    html_client = render_ia_template("example.html", context=example_context, user=user_client, lang=lang)
    assert "Administrateur" in html_admin or "Admin" in html_admin or "مشرف" in html_admin or "ⴰⵎⴰⵏⴰⵡⴰⵏ" in html_admin
    assert "Data Scientist" in html_ds or "Scientifique" in html_ds or "عالم بيانات" in html_ds or "ⴰⴷⴰⵜⴰ ⵙⴽⵉⵏⵜⵉⴼⴰⵍⴰⵢ" in html_ds
...
def test_render_ia_template_not_exist():
    with pytest.raises(TemplateDoesNotExist):
        render_ia_template("notfound.html")

@pytest.mark.parametrize("lang", ["fr", "en", "ar", "tz"])
def test_validate_ia_template_compliance(lang, example_context):
    html = render_ia_template("example.html", context=example_context, lang=lang)
    results = validate_ia_template(html, lang=lang)
...
def test_validate_ia_template_security_fail():
    html = "<html lang='fr'><body><script>alert('xss')</script></body></html>"
    results = validate_ia_template(html, lang="fr")
    assert results["security"] is False
    assert any("Script inline" in str(e) for e in results["errors"])

def test_validate_ia_template_accessibility_fail():
    html = "<html><body></body></html>"
    results = validate_ia_template(html, lang="fr")
    assert results["accessibility"] is False
    assert any("lang" in str(e) for e in results["errors"])

def test_validate_ia_template_privacy_fail():
    html = "<html lang='fr'><body>{{ ai_model.api_key }}</body></html>"
    results = validate_ia_template(html, lang="fr")
    assert results["privacy"] is False
    assert any("IA" in str(e) or "confidentielles" in str(e) for e in results["errors"])
...
def test_end_to_end_template_render_and_validate(lang, user_client, example_context):
    html = render_ia_template("example.html", context=example_context, user=user_client, lang=lang)
    results = validate_ia_template(html, lang=lang)
...
def test_template_env_var_disable_check(monkeypatch):
    ...
    import importlib
    import sys
    mod = importlib.import_module("Dihya.backend.django.app.templates.intelligence_artificielle.__init__")
...
def test_template_render_with_missing_context_keys(user_client):
    ...
    html = render_ia_template("example.html", context={}, user=user_client, lang="fr")
    assert "<html" in html
...
def test_template_render_with_special_characters(user_client, example_context):
    ...
    html = render_ia_template("example.html", context=example_context, user=user_client, lang="fr")
...
# --- Fin des tests intelligence_artificielle ---
