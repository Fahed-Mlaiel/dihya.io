# 🚗 Dihya – Politique d’Utilisation & Sécurité des Templates Automobile
...
Garantir la sécurité, la souveraineté, la conformité et la qualité des templates métiers utilisés pour l’automobile (fiches véhicules, diagnostics, contrats, sinistres, IA, plugins) dans la plateforme Dihya.
...
- **Formats acceptés** : .json, .yaml, .yml, .py, .md, .pdf (fiches, diagnostics, scripts de génération uniquement)
...
- **Tests d’intégrité** : chaque template doit passer les scripts de validation fournis (`validate_automobile_template.py`)
...
- **Séparer** les templates par usage (fiches, diagnostics, contrats, sinistres, scripts)
