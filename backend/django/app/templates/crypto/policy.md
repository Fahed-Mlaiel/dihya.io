# 🔐 Dihya – Politique d’Utilisation & Sécurité des Templates Crypto
...
Garantir la sécurité, la souveraineté, la conformité et la qualité des templates métiers utilisés pour la cryptographie (protocoles, clés, transactions, audits, wallets, IA, plugins) dans la plateforme Dihya.
...
- **Formats acceptés** : .json, .yaml, .yml, .py, .md, .pdf (protocoles, clés, scripts de génération uniquement)
...
- **Tests d’intégrité** : chaque template doit passer les scripts de validation fournis (`validate_crypto_template.py`)
...
- **Séparer** les templates par usage (protocoles, clés, transactions, audits, wallets, scripts)
