# 💄 Dihya – Politique d’Utilisation & Sécurité des Templates Beauté & Bien-être
...
Garantir la sécurité, la souveraineté, la conformité et la qualité des templates métiers utilisés pour la beauté et le bien-être (fiches soins, diagnostics, routines, produits, IA, plugins) dans la plateforme Dihya.
...
- **Formats acceptés** : .json, .yaml, .yml, .py, .md, .pdf (fiches, routines, scripts de génération uniquement)
...
- **Tests d’intégrité** : chaque template doit passer les scripts de validation fournis (`validate_beaute_template.py`)
...
- **Séparer** les templates par usage (fiches, diagnostics, routines, produits, scripts)
