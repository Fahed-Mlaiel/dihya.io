...
"""
Tests avancés Dihya – test_verif_metiers_tests.py
...
def test_integration_multilingue(tmp_path):
    ...
    (tmp_path / "routes" / "industrie").mkdir(parents=True)
    (tmp_path / "routes" / "industrie" / "routes.py").write_text("# route industrie")
    metiers, _, _ = detect_metiers_arbo(str(tmp_path), show_paths=True)
    assert any(m in ["industrie", "صناعة", "ⵙⵉⵏⴷⵉⵔⵢ"] for m in metiers)
