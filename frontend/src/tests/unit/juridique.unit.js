/**
 * @file juridique.unit.js
 * @description Tests unitaires avancés pour la gestion de projets juridiques (REST, GraphQL, sécurité, i18n, rôles, audit)
...
describe('JuridiqueService', () => {
  beforeAll(() => enableAudit());
  it('devrait créer un projet juridique (admin)', async () => {
    const i18n = getI18n('fr');
    const service = new JuridiqueService(mockAdmin, i18n);
    const projet = await service.createProject({ nom: 'Dihya Legal', pays: 'Algérie' });
    expect(projet.nom).toBe('Dihya Legal');
    expect(projet.ownerRole).toBe('admin');
  });
  it('should deny creation for guest', async () => {
    const i18n = getI18n('en');
    const service = new JuridiqueService(mockGuest, i18n);
  ...
  it('doit logger chaque action (audit)', async () => {
    const i18n = getI18n('fr');
    const service = new JuridiqueService(mockUser, i18n);
