/**
 * @file logistique.unit.js
 * @description Tests unitaires avancés pour la gestion de projets logistique (REST, GraphQL, sécurité, i18n, rôles, audit)
...
describe('LogistiqueService', () => {
  ...
  it('devrait créer un projet logistique (admin)', async () => {
    const i18n = getI18n('fr');
    const service = new LogistiqueService(mockAdmin, i18n);
    const projet = await service.createProject({ nom: 'Dihya Log', pays: 'Algérie' });
    expect(projet.nom).toBe('Dihya Log');
    expect(projet.ownerRole).toBe('admin');
  });
  it('should deny creation for guest', async () => {
    const i18n = getI18n('en');
    const service = new LogistiqueService(mockGuest, i18n);
  ...
  it('doit logger chaque action (audit)', async () => {
    const i18n = getI18n('fr');
    const service = new LogistiqueService(mockUser, i18n);
