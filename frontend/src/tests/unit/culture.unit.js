/**
 * @file culture.unit.js
 * @description Tests unitaires avancés pour la gestion de projets culturels (REST, GraphQL, sécurité, i18n, rôles, audit)
...
describe('CultureService', () => {
  ...
  it('devrait créer un projet culturel (admin)', async () => {
    const i18n = getI18n('fr');
    const service = new CultureService(mockAdmin, i18n);
    const projet = await service.createProject({ nom: 'Festival Dihya', pays: 'Algérie' });
    expect(projet.nom).toBe('Festival Dihya');
    expect(projet.ownerRole).toBe('admin');
  });
  it('should deny creation for guest', async () => {
    const i18n = getI18n('en');
    const service = new CultureService(mockGuest, i18n);
  ...
  it('doit logger chaque action (audit)', async () => {
    const i18n = getI18n('fr');
    const service = new CultureService(mockUser, i18n);
