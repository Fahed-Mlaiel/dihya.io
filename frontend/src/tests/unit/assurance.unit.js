/**
 * @file Unit-Tests für das Assurance-Modul (Dihya Coding)
...
import { AssuranceForm } from '../../../../src/components/metiers/assurance/index.js';

describe('AssuranceForm', () => {
  it('rendert korrekt auf Englisch', () => {
    render(<AssuranceForm lang="en" />);
    expect(screen.getByRole('form', { name: /insurance/i })).toBeInTheDocument();
  });
  it('rendert korrekt auf Französisch', () => {
    render(<AssuranceForm lang="fr" />);
    expect(screen.getByRole('form', { name: /assurance/i })).toBeInTheDocument();
  });
  ...
  it('unterstützt Plugins (Mock)', () => {
    ...
    render(<AssuranceForm lang="en" plugin={plugin} />);
    expect(plugin).not.toThrow;
  });
  it('respektiert RBAC (Mock)', () => {
    const user = { role: 'guest' };
    render(<AssuranceForm lang="en" user={user} />);
