/**
 * @file loisirs.unit.js
 * @description Tests unitaires avancés pour la gestion de projets loisirs (REST, GraphQL, sécurité, i18n, rôles, audit)
...
import { LoisirsService } from '../../services/loisirsService';
import { enableAudit, getAuditLogs } from '../../utils/audit';
import { getI18n } from '../../utils/i18n';

describe('LoisirsService', () => {
  beforeAll(() => enableAudit());
  it('devrait créer un projet loisirs (admin)', async () => {
    const i18n = getI18n('fr');
    const service = new LoisirsService(mockAdmin, i18n);
    const projet = await service.createProject({ nom: 'Dihya Fun', pays: 'Algérie' });
    expect(projet.nom).toBe('Dihya Fun');
    expect(projet.ownerRole).toBe('admin');
  });
  it('should deny creation for guest', async () => {
    const i18n = getI18n('en');
    const service = new LoisirsService(mockGuest, i18n);
  ...
  it('doit logger chaque action (audit)', async () => {
    const i18n = getI18n('fr');
    const service = new LoisirsService(mockUser, i18n);
