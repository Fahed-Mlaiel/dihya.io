/**
 * @file journalisme.unit.js
 * @description Tests unitaires avancés pour la gestion de projets journalisme (REST, GraphQL, sécurité, i18n, rôles, audit)
...
import { JournalismeService } from '../../services/journalismeService';
import { enableAudit, getAuditLogs } from '../../utils/audit';
import { getI18n } from '../../utils/i18n';

describe('JournalismeService', () => {
  beforeAll(() => enableAudit());
  it('devrait créer un projet journalisme (admin)', async () => {
    const i18n = getI18n('fr');
    const service = new JournalismeService(mockAdmin, i18n);
    const projet = await service.createProject({ nom: 'Dihya News', pays: 'Algérie' });
    expect(projet.nom).toBe('Dihya News');
    expect(projet.ownerRole).toBe('admin');
  });
  it('should deny creation for guest', async () => {
    const i18n = getI18n('en');
    const service = new JournalismeService(mockGuest, i18n);
  ...
  it('doit logger chaque action (audit)', async () => {
    const i18n = getI18n('fr');
    const service = new JournalismeService(mockUser, i18n);
