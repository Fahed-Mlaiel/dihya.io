...
## Objectif
Tests avancés pour l’API REST/GraphQL de gestion du sport (clubs, fédérations, IA, VR, AR, etc.)

- Sécurité maximale (CORS, JWT, WAF, anti-DDOS, audit, anonymisation RGPD)
- Multitenancy, gestion des rôles (admin, coach, joueur, invité)
...
## Structure des tests
- `test_sport.js` : tests d’intégration API (GET, POST, sécurité, i18n, plugins, RGPD, audit)
...
```bash
npm run test:integration -- --testPathPattern=sport
```
...
# Integration Tests – Sport (English)
