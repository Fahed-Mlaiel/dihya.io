// test_mode.js
/**
 * @file Test d'intégration avancé pour la gestion des projets mode (IA, VR, AR...)
 * Sécurité maximale, i18n, multitenancy, IA fallback, SEO, plugins, RGPD, auditabilité
 * Tests complets (unit, integration, e2e)
 */
const request = require('supertest');
const app = require('../../../app');
const { getJWT, getI18nHeaders, getRoleHeaders } = require('../../utils/testUtils');

describe('Mode API Integration', () => {
  it('should create a mode project (admin, zh)', async () => {
    const res = await request(app)
      .post('/api/v1/mode')
      .set(getJWT('admin'))
      .set(getI18nHeaders('zh'))
      .send({ name: '时尚项目', type: 'AI', lang: 'zh' });
    expect(res.statusCode).toBe(201);
    expect(res.body).toHaveProperty('id');
    expect(res.body.lang).toBe('zh');
  });
  // ...autres tests multilingues, rôles, sécurité, fallback IA, SEO, plugins, RGPD, auditabilité
});
