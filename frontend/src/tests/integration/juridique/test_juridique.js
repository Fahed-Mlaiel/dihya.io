// test_juridique.js
/**
 * @file Tests d'intégration pour la gestion de projets Juridique (REST & GraphQL)
...
describe('Juridique - Intégration API', () => {
  let jwt, tenant;
  beforeAll(async () => {
    tenant = await getTestTenant('juridique');
    jwt = await getJWT({ role: 'admin', tenant });
  });

  it('crée un projet juridique (REST)', async () => {
    const res = await request(app)
      .post(`/api/v1/juridique/projects`)
  ...
  it('liste les projets (GraphQL)', async () => {
    const query = `query { juridiqueProjects { id name type lang } }`;
    const res = await request(app)
      .post('/graphql')
      .set('Authorization', `Bearer ${jwt}`)
    ...
    expect(res.body.data.juridiqueProjects).toBeInstanceOf(Array);
  });

  it('refuse accès sans JWT', async () => {
    const res = await request(app)
      .get(`/api/v1/juridique/projects`)
      .set(getI18nHeaders('fr'));
